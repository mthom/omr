...
static intptr_t ensureBaseFile(struct OMRPortLibrary *portLibrary, char *filename);
static intptr_t getSemHandle(struct OMRPortLibrary * portLibrary, const char * baseFile, int setSize, uint32_t permissions, uint8_t projid, omrshsem_handle **handle);
static omrshsem_handle* createsemHandle(struct OMRPortLibrary *portLibrary, int semid, int nsems, const char* baseFile);
...
omrshsem_open (struct OMRPortLibrary *portLibrary, struct omrshsem_handle **handle, const struct OMRPortShSemParameters *params) 
{
	...
	*handle = NULL;
	...
	if (ensureBaseFile(portLibrary, baseFile) != OMRSH_SUCCESS) {
...
static intptr_t 
ensureBaseFile(struct OMRPortLibrary *portLibrary, char *filename) 
{
	intptr_t fd;
	intptr_t rc = 0;
	gid_t gid = getegid();
...
static intptr_t 
getSemHandle(struct OMRPortLibrary * portLibrary, const char * baseFile, int setSize, uint32_t permissions, uint8_t projid, omrshsem_handle **handle)
{
	/*ftok variables*/
	key_t fkey;
...
createsemHandle(struct OMRPortLibrary *portLibrary, int semid, int nsems, const char* baseFile) 
{
