...
static int32_t createSharedMemory(OMRPortLibrary *portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, const char *baseFile, int32_t size, int32_t perm, struct omrshmem_controlFileFormat * controlinfo, uintptr_t groupPerm, struct omrshmem_handle *handle);
static intptr_t checkSize(OMRPortLibrary *portLibrary, int shmid, int64_t size);
static intptr_t openSharedMemory (OMRPortLibrary *portLibrary, intptr_t fd, const char *baseFile, int32_t perm, struct omrshmem_controlFileFormat * controlinfo, uintptr_t groupPerm, struct omrshmem_handle *handle, BOOLEAN *canUnlink, uintptr_t cacheFileType);
static void getControlFilePath(struct OMRPortLibrary* portLibrary, const char* cacheDirName, char* buffer, uintptr_t size, const char* name);
static void createshmHandle(OMRPortLibrary *portLibrary, int32_t shmid, const char *controlFile, OMRMemCategory * category, uintptr_t size, omrshmem_handle * handle, uint32_t perm);
static intptr_t checkGid(OMRPortLibrary *portLibrary, int shmid, int32_t gid);
static intptr_t checkUid(OMRPortLibrary *portLibrary, int shmid, int32_t uid);
static uintptr_t isControlFileName(struct OMRPortLibrary* portLibrary, char* buffer);
static intptr_t readControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshmem_controlFileFormat * info);
static intptr_t readOlderNonZeroByteControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshmem_controlBaseFileFormat * info);
static void initShmemStatsBuffer(OMRPortLibrary *portLibrary, OMRPortShmemStatistic *statbuf);
static intptr_t getShmStats(OMRPortLibrary *portLibrary, int shmid, struct OMRPortShmemStatistic* statbuf);

/* In omrshchelp.c */
extern uintptr_t isCacheFileName(OMRPortLibrary *, const char *, uintptr_t, const char *);
...
omrshmem_openDeprecated (struct OMRPortLibrary *portLibrary, const char* cacheDirName, uintptr_t groupPerm, struct omrshmem_handle **handle, const char* rootname, uint32_t perm, uintptr_t cacheFileType, uint32_t categoryCode)
{
	...
	getControlFilePath(portLibrary, cacheDirName, baseFile, OMRSH_MAXPATH, rootname);
	...
	if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, baseFile, 0) != OMRSH_SUCCESS) {
	...
	if (cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
		...
		int projid = 0xde;
		key_t fkey;
//		omrshmem_controlFileFormat controlinfo;
		uintptr_t size = 0;
		...
		retval = openSharedMemory(portLibrary, fd, baseFile, perm, (omrshmem_controlFileFormat *)&controlinfo, groupPerm, tmphandle, &canUnlink, cacheFileType);
...
error:
	if (fileIsLocked == TRUE) {
		if (ControlFileCloseAndUnLock(portLibrary, fd) != OMRSH_SUCCESS) {
...
omrshmem_open (OMRPortLibrary *portLibrary, const char* cacheDirName, uintptr_t groupPerm, struct omrshmem_handle **handle, const char *rootname, uintptr_t size, uint32_t perm, uint32_t categoryCode, uintptr_t flags, OMRControlFileStatus *controlFileStatus)
{
	...
	*handle = NULL;
	...
	for (retryIfReadOnlyCount = 10; retryIfReadOnlyCount > 0; retryIfReadOnlyCount -= 1) {
		...
		if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, canCreateFile, baseFile, groupPerm) != OMRSH_SUCCESS) {
		...
		if (rc == OMRSH_ERROR) {
				...
				if (rc == OMRPORT_ERROR_SHMEM_OPFAILED) {
					...
					if (perm == OMRSH_SHMEM_PERM_READ) {
						...
						if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
							Trc_PRT_shmem_omrshmem_open_Msg("Info: Control file is unlinked after call to createSharedMemory.");
						} else {
							Trc_PRT_shmem_omrshmem_open_Msg("Info: Failed to unlink control file after call to createSharedMemory.");
						}
					} else {
			...
			rc = openSharedMemory(portLibrary, fd, baseFile, perm, &controlinfo, groupPerm, tmphandle, &canUnlink, OMRSH_SYSV_REGULAR_CONTROL_FILE);
				...
				if ((TRUE == canUnlink)
					...
					if (perm == OMRSH_SHMEM_PERM_READ) {
						...
						if ((FALSE == isReadOnlyFD)
							...
							if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
								Trc_PRT_shmem_omrshmem_open_Msg("Info: Control file was unlinked after call to openSharedMemory.");
								...
								Trc_PRT_shmem_omrshmem_open_Msg("Info: Failed to unlink control file after call to openSharedMemory.");
							}
						} else {
							Trc_PRT_shmem_omrshmem_open_Msg("Info: Control file was not unlinked after openSharedMemory because it is read-only and (its modLevel does not allow unlinking or rc != SHARED_MEMORY_NOT_FOUND).");
						}
					}
				} else {
					Trc_PRT_shmem_omrshmem_open_Msg("Info: Control file was not unlinked after call to openSharedMemory.");
				}
			}
		} else {
		...
		if ((OMRPORT_INFO_SHMEM_CREATED != rc) && (OMRPORT_INFO_SHMEM_OPENED != rc)) {
			...
			if (((isReadOnlyFD == TRUE) || (rc == OMRPORT_INFO_SHMEM_OPEN_UNLINKED)) && (retryIfReadOnlyCount > 1)) {
				...
				omrmem_free_memory(portLibrary, tmphandle);
				/* Here 'rc' can be:
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED (returned either by createSharedMemory or openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_CONTROL_FILE_CORRUPT
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHMID_MISMATCH (returned by openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHM_KEY_MISMATCH (returned by openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHM_GROUPID_CHECK_FAILED (returned by openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHM_USERID_CHECK_FAILED (returned by openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHM_SIZE_CHECK_FAILED (returned by openSharedMemory)
				 * 	- OMRPORT_ERROR_SHMEM_OPFAILED_SHARED_MEMORY_NOT_FOUND (returned by openSharedMemory)
				 */
				return rc;
			}
		}
...
//	intptr_t rcunlink;
	...
	if (cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
		Trc_PRT_shmem_destroyDeprecated_Message("Info: cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE.");

		if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, (*handle)->baseFileName, 0) != OMRSH_SUCCESS) {
	...
	error:
		...
		Trc_PRT_shmem_destroyDeprecated_Message("Info: cacheFileType == OMRSH_SYSV_OLDER_CONTROL_FILE.");
...
omrshmem_stat(struct OMRPortLibrary *portLibrary, const char* cacheDirName, uintptr_t groupPerm, const char* name, struct OMRPortShmemStatistic* statbuf)
{
	...
	getControlFilePath(portLibrary, cacheDirName, controlFile, OMRSH_MAXPATH, name);
	...
	/*Try to read data from the file*/
	if (OMRSH_SUCCESS != readControlFile(portLibrary, fd, &controlinfo)) {
...
omrshmem_statDeprecated (struct OMRPortLibrary *portLibrary, const char* cacheDirName, uintptr_t groupPerm, const char* name, struct OMRPortShmemStatistic* statbuf, uintptr_t cacheFileType)
{
	...
	getControlFilePath(portLibrary, cacheDirName, controlFile, OMRSH_MAXPATH, name);
	if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, controlFile, 0) != OMRSH_SUCCESS) {
	...
	if (cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
		...
		int projid = 0xde;
		key_t fkey;
//		omrshmem_controlFileFormat info;
		...
		if (fkey == -1) {
			Trc_PRT_shmem_omrshmem_statDeprecated_Message("Error: ftokWrapper failed.");
			retval = OMRSH_FAILED;
			goto error;
		}
		shmflags &= ~IPC_CREAT;
...
#endif
		if (shmid == -1) {
			Trc_PRT_shmem_omrshmem_statDeprecated_Message("Error: shmgetWrapper failed.");
			retval = OMRSH_FAILED;
			goto error;
		}
		statbuf->shmid = shmid;
		...
		if (readOlderNonZeroByteControlFile(portLibrary, fd, &info) != OMRSH_SUCCESS) {
	...
	rc = getShmStats(portLibrary, statbuf->shmid, statbuf);
	if (OMRPORT_INFO_SHMEM_STAT_PASSED != rc) {
		Trc_PRT_shmem_omrshmem_statDeprecated_Message("Error: getShmStats failed");
		retval = OMRSH_FAILED;
		goto error;
	}
 ...
 */
static void
createshmHandle(OMRPortLibrary *portLibrary, int32_t shmid, const char *controlFile, OMRMemCategory * category, uintptr_t size, omrshmem_handle * handle, uint32_t perm)
{
	...
	Trc_PRT_shmem_createshmHandle_Exit(OMRSH_SUCCESS);
}
...
static uintptr_t
isControlFileName(struct OMRPortLibrary* portLibrary, char* nameToTest)
{
	return isCacheFileName(portLibrary, nameToTest, OMRPORT_SHR_CACHE_TYPE_NONPERSISTENT, OMRSH_MEMORY_ID);
}
 ...
 */
static void 
getControlFilePath(struct OMRPortLibrary* portLibrary, const char* cacheDirName, char* buffer, uintptr_t size, const char* name)
{
...
omrshmem_createDir(struct OMRPortLibrary* portLibrary, char* cacheDirName, uintptr_t cacheDirPerm, BOOLEAN cleanMemorySegments)
{
	...
	case EsIsDir:
		...
		if (OMRSH_DIRPERM_ABSENT == cacheDirPerm) {
			...
			rc2 = changeDirectoryPermission(portLibrary, cacheDirName, OMRSH_DIRPERM);
...
static intptr_t
readControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshmem_controlFileFormat * info)
{
...
static intptr_t 
readOlderNonZeroByteControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshmem_controlBaseFileFormat * info)
{
 ...
 */
static intptr_t 
openSharedMemory (OMRPortLibrary *portLibrary, intptr_t fd, const char *baseFile, int32_t perm, struct omrshmem_controlFileFormat * controlinfo, uintptr_t groupPerm, struct omrshmem_handle *handle, BOOLEAN *canUnlink, uintptr_t cacheFileType)
{
	...
	do {
		...
		if (OMRSH_SYSV_REGULAR_CONTROL_FILE == cacheFileType) {
			...
			if (rc == 0) {
				...
				Trc_PRT_shmem_omrshmem_openSharedMemory_ExitWithMessage("Error: checkGid failed during shmctl.");
				rc = OMRPORT_ERROR_SHMEM_OPFAILED;
				goto failDontUnlink;
			} /*Continue looking for opportunity to re-create*/
			...
			if (rc == 0) {
				...
				Trc_PRT_shmem_omrshmem_openSharedMemory_ExitWithMessage("Error: checkUid failed during shmctl.");
				rc = OMRPORT_ERROR_SHMEM_OPFAILED;
				goto failDontUnlink;
			} /*Continue looking for opportunity to re-create*/
			...
			if (rc == 0) {
				...
				Trc_PRT_shmem_omrshmem_openSharedMemory_ExitWithMessage("Error: checkSize failed during shmctl.");
				rc = OMRPORT_ERROR_SHMEM_OPFAILED;
				goto failDontUnlink;
			} /*Continue looking for opportunity to re-create*/
		}
 ...
 */
static int32_t 
createSharedMemory(OMRPortLibrary *portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, const char *baseFile, int32_t size, int32_t perm, struct omrshmem_controlFileFormat * controlinfo, uintptr_t groupPerm, struct omrshmem_handle *handle)
{
 ...
 */
static intptr_t 
checkSize(OMRPortLibrary *portLibrary, int shmid, int64_t size)
{
 ...
 */
static intptr_t 
checkUid(OMRPortLibrary *portLibrary, int shmid, int32_t uid)
{
 ...
 */
static intptr_t 
checkGid(OMRPortLibrary *portLibrary, int shmid, int32_t gid)
{
 ...
 */
static void
initShmemStatsBuffer(OMRPortLibrary *portLibrary, OMRPortShmemStatistic *statbuf)
{
	if (NULL != statbuf) {
 ...
 */
static intptr_t
getShmStats(OMRPortLibrary *portLibrary, int shmid, struct OMRPortShmemStatistic* statbuf)
{
	intptr_t rc = OMRPORT_ERROR_SHMEM_STAT_FAILED;
	struct shmid_ds shminfo;
