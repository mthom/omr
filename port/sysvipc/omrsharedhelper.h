...
void clearPortableError(OMRPortLibrary *portLibrary);
intptr_t ControlFileOpenWithWriteLock(struct OMRPortLibrary* portLibrary, intptr_t * fd, BOOLEAN * isReadOnlyFD, BOOLEAN canCreateNewFile, const char * filename, uintptr_t groupPerm);
intptr_t ControlFileCloseAndUnLock(struct OMRPortLibrary* portLibrary, intptr_t fd);
intptr_t changeDirectoryPermission(struct OMRPortLibrary *portLibrary, const char* pathname, uintptr_t permission);
void cleanSharedMemorySegments(struct OMRPortLibrary* portLibrary);
intptr_t createDirectory(struct OMRPortLibrary *portLibrary, char *pathname, uintptr_t permission);
BOOLEAN unlinkControlFile(struct OMRPortLibrary* portLibrary, const char *controlFile, OMRControlFileStatus *status);
...
#if defined(AIXPPC)
uintptr_t isPageProtectionPossible(struct OMRPortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm, uintptr_t regionSize);
#endif
