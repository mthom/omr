...
static intptr_t checkMarker(OMRPortLibrary *portLibrary, int semid, int semsetsize);
static intptr_t checkSize(OMRPortLibrary *portLibrary, int semid, int32_t numsems);
static intptr_t createSemaphore(struct OMRPortLibrary *portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, char *baseFile, int32_t setSize, omrshsem_baseFileFormat* controlinfo, uintptr_t groupPerm);
static intptr_t openSemaphore(struct OMRPortLibrary *portLibrary, intptr_t fd, char *baseFile, omrshsem_baseFileFormat* controlinfo, uintptr_t groupPerm, uintptr_t cacheFileType);
static void createsemHandle(struct OMRPortLibrary *portLibrary, int semid, int nsems, char* baseFile, omrshsem_handle * handle);
static intptr_t readOlderNonZeroByteControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshsem_baseFileFormat * info);
static void initShsemStatsBuffer(OMRPortLibrary *portLibrary, OMRPortShsemStatistic *statbuf);
	...
	*handle = NULL;
	...
	for (retryIfReadOnlyCount = 10; retryIfReadOnlyCount > 0; retryIfReadOnlyCount -= 1) {
		...
		if (rc == -1) {
				...
				if (rc == OMRPORT_ERROR_SHSEM_OPFAILED) {
					...
					if (isReadOnlyFD == FALSE) {
						if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
							Trc_PRT_shsem_omrshsem_open_Msg("Info: Control file is unlinked after call to createSemaphore.");
						} else {
							Trc_PRT_shsem_omrshsem_open_Msg("Info: Failed to unlink control file after call to createSemaphore.");
						}
					} else {
						Trc_PRT_shsem_omrshsem_open_Msg("Info: Control file is not unlinked after call to createSemaphore(isReadOnlyFD == TRUE).");
					}
				}
			} else {
			...
			rc = openSemaphore(portLibrary, fd, baseFile, &controlinfo, groupPerm, OMRSH_SYSV_REGULAR_CONTROL_FILE);
			if (OMRPORT_INFO_SHSEM_OPENED != rc) {
				...
				if ((OMRPORT_ERROR_SHSEM_OPFAILED_DONT_UNLINK != rc)
					...
					if ((FALSE == isReadOnlyFD)
						...
						if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
							Trc_PRT_shsem_omrshsem_open_Msg("Info: Control file is unlinked after call to openSemaphore.");
						...
						Trc_PRT_shsem_omrshsem_open_Msg("Info: Control file is not unlinked after openSemaphore because it is read-only and (its modLevel does not allow unlinking or rc != SEMAPHORE_NOT_FOUND).");
					}
				} else {
					Trc_PRT_shsem_omrshsem_open_Msg("Info: Control file is not unlinked after call to openSemaphore.");
				}
			}
		} else {
		...
		if ((OMRPORT_INFO_SHSEM_CREATED != rc) && (OMRPORT_INFO_SHSEM_OPENED != rc)) {
			...
			if (((isReadOnlyFD == TRUE) || (rc == OMRPORT_INFO_SHSEM_OPEN_UNLINKED)) && (retryIfReadOnlyCount > 1)) {
				...
				/* Here 'rc' can be:
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED (returned either by createSemaphore or openSemaphore)
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED_CONTROL_FILE_CORRUPT
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED_SEMID_MISMATCH (returned by openSemaphore)
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED_SEM_KEY_MISMATCH (returned by openSemaphore)
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED_SEM_SIZE_CHECK_FAILED (returned by openSemaphore)
				 * 	- OMRPORT_ERROR_SHSEM_OPFAILED_SEM_MARKER_CHECK_FAILED (returned by openSemaphore)
				 *  - OMRPORT_ERROR_SHSEM_OPFAILED_SEMAPHORE_NOT_FOUND ( returned by openSemaphore)
				 */
				return rc;
			}
		}
...
//	intptr_t rcunlink;
	...
	if (cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
		Trc_PRT_shsem_deprecated_destroyDeprecated_Message("Info: cacheFileType == OMRSH_SYSV_OLDER_EMPTY_CONTROL_FILE.");
		if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, (*handle)->baseFile, 0) != OMRSH_SUCCESS) {
	...
	error:
		...
		Trc_PRT_shsem_deprecated_destroyDeprecated_Message("Info: cacheFileType == OMRSH_SYSV_OLDER_CONTROL_FILE.");
...
static intptr_t 
readOlderNonZeroByteControlFile(OMRPortLibrary *portLibrary, intptr_t fd, omrshsem_baseFileFormat * info)
{
	intptr_t rc;
 ...
 */
static intptr_t
createSemaphore(struct OMRPortLibrary *portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, char *baseFile, int32_t setSize, omrshsem_baseFileFormat* controlinfo, uintptr_t groupPerm)
{
 ...
 */
static intptr_t
openSemaphore(struct OMRPortLibrary *portLibrary, intptr_t fd, char *baseFile, omrshsem_baseFileFormat* controlinfo, uintptr_t groupPerm, uintptr_t cacheFileType)
{
	/*The control file contains data, and we have successfully read in our structure*/
	int semid = -1;
 ...
 */
static void
createsemHandle(struct OMRPortLibrary *portLibrary, int semid, int nsems, char* baseFile, omrshsem_handle * handle) 
{
	...
	Trc_PRT_shsem_createsemHandle_Exit(OMRSH_SUCCESS);
}
 ...
 */
static intptr_t 
checkMarker(OMRPortLibrary *portLibrary, int semid, int semsetsize)
{
	intptr_t rc;
 ...
 */
static intptr_t 
checkSize(OMRPortLibrary *portLibrary, int semid, int32_t numsems)
{
	intptr_t rc;
	union semun semctlArg;
	struct semid_ds buf;
 ...
 */
static void
initShsemStatsBuffer(OMRPortLibrary *portLibrary, OMRPortShsemStatistic *statbuf)
{
	if (NULL != statbuf) {
