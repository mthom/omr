...
typedef struct Dummy_OSCache_header {
	char eyecatcher[OMRPORT_SHMEM_EYECATCHER_LENGTH+1]; /* "J9SC" */
	uintptr_t version;
...
void
clearPortableError(OMRPortLibrary *portLibrary) {
   omrerror_set_last_error(portLibrary, 0, 0);
}

intptr_t
changeDirectoryPermission(struct OMRPortLibrary *portLibrary, const char* pathname, uintptr_t permission)
{
	struct stat statbuf;
...
intptr_t
createDirectory(struct OMRPortLibrary *portLibrary, char *pathname, uintptr_t permission)
{
...
void
cleanSharedMemorySegments(struct OMRPortLibrary* portLibrary)
{
...
#define OUTPUTBUFSIZE 256
  ...
  //	OMRPortLibrary *omrPortLibrary = OMRPORT_FROM_J9PORT(portLibrary);
	...
	portLibrary->sysinfo_get_username(portLibrary, processOwner, OUTPUTBUFSIZE);
...
#pragma convlit(resume)
	...
	while (NULL != fgets( outputBuf, OUTPUTBUFSIZE, ipcsOutput)) {
		...
		if (ipcsField[IPCS_USERID_FIELD_NO] && 
			...
			char *region = NULL;
			char *sharedClassesEyecatcher = OMRPORT_SHMEM_EYECATCHER;
			int sharedClassesEyecatcherLength = OMRPORT_SHMEM_EYECATCHER_LENGTH;
 ...
 */
intptr_t 
ControlFileOpenWithWriteLock(struct OMRPortLibrary* portLibrary, intptr_t * fd, BOOLEAN * isReadOnlyFD, BOOLEAN canCreateNewFile, const char * filename, uintptr_t groupPerm)
{
...
intptr_t
ControlFileCloseAndUnLock(struct OMRPortLibrary* portLibrary, intptr_t fd)
{
 ...
 */
BOOLEAN
unlinkControlFile(struct OMRPortLibrary* portLibrary, const char *controlFile, OMRControlFileStatus *controlFileStatus)
{
