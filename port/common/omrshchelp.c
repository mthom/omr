 ...
 */
uintptr_t
getValuesFromShcFilePrefix(struct OMRPortLibrary* portLibrary, const char* filename, struct J9PortShcVersion* result) 
{
	if (filename != NULL) {
		uintptr_t temp;
		char* cursor = (char*)filename;

		if (*cursor == OMRSH_VERSION_PREFIX_CHAR) {
			++cursor;
		...
		if ((*cursor == OMRSH_MODLEVEL_PREFIX_CHAR) || (*cursor == OMRSH_MODLEVEL_G07ANDLOWER_CHAR)) {
			++cursor;
			if (scan_udata(&cursor, &temp) == 0) {
		...
		if (*cursor == OMRSH_FEATURE_PREFIX_CHAR) {
			++cursor;
		...
		if (*cursor == OMRSH_ADDRMODE_PREFIX_CHAR) {
			++cursor;
			if (scan_udata(&cursor, &temp) == 0) {
		...
		if (*cursor == OMRSH_PERSISTENT_PREFIX_CHAR) {
			result->cacheType = OMRPORT_SHR_CACHE_TYPE_PERSISTENT;
			++cursor;
		} else if (OMRSH_SNAPSHOT_PREFIX_CHAR == *cursor) {
			result->cacheType = OMRPORT_SHR_CACHE_TYPE_SNAPSHOT;
			++cursor;
		} else {
			result->cacheType = OMRPORT_SHR_CACHE_TYPE_NONPERSISTENT;
		}
		if (*cursor != OMRSH_PREFIX_SEPARATOR_CHAR) {
			return 0;
		}
	} else {
		return 0;
...
getModLevelFromName(const char* cacheNameWithVGen)
{
	...
	if ((strlen(cacheNameWithVGen) > (OMRSH_MODLEVEL_PREFIX_CHAR_OFFSET + 2))
		 && (OMRSH_MODLEVEL_PREFIX_CHAR == cursor[OMRSH_MODLEVEL_PREFIX_CHAR_OFFSET])
	) {
		cursor += (OMRSH_MODLEVEL_PREFIX_CHAR_OFFSET + 1);
...
//	switch (j2seVersion) {
//	case J2SE_18 :
//		modLevel = OMRSH_MODLEVEL_JAVA8;
//		break;
//	case J2SE_19 :
//		modLevel = OMRSH_MODLEVEL_JAVA9;
//		break;
//	default: 
...
getJCLForShcModlevel(uintptr_t modlevel)
{
	...
	switch (modlevel) {
	case OMRSH_MODLEVEL_JAVA5 :
		javaVersion = 5;
		break;
	case OMRSH_MODLEVEL_JAVA6 :
		javaVersion = 6;
		break;
	case OMRSH_MODLEVEL_JAVA7 :
		javaVersion = 7;
		break;
	case OMRSH_MODLEVEL_JAVA8 :
		javaVersion = 8;
		break;
	case OMRSH_MODLEVEL_JAVA9 :
		javaVersion = 9;
		break;
	case OMRSH_MODLEVEL_JAVA10:
	...
	default:
		if (modlevel >= 10) {
			/* OMRSH_MODLEVEL_JAVA9 is 5. Does not have modlevel that is 7,8,9 */
 ...
 */
uintptr_t 
isCompatibleShcFilePrefix(OMRPortLibrary* portlib, uint32_t javaVersion, uint32_t feature, const char* filename)
{
...
//			(versionData.addrmode == OMRSH_ADDRMODE) &&
 ...
 */
void
getStringForShcModlevel(OMRPortLibrary* portlib, uint32_t modlevel, char* buffer, uint32_t buffSize)
{
	switch (modlevel) {
	case OMRSH_MODLEVEL_JAVA5 :
	...
	default :
		if (modlevel >= 10) {
			portlib->str_printf(portlib, buffer, buffSize, "%s%u", "Java", modlevel);
		} else {
			/* OMRSH_MODLEVEL_JAVA9 is 5. Does not have modlevel that is 7,8,9 */
 ...
 */
void
getStringForShcAddrmode(OMRPortLibrary* portlib, uint32_t addrmode, char* buffer)
{
	switch (addrmode) {
	case OMRSH_ADDRMODE_32 :
		strcpy(buffer, "32-bit");
		break;
	case OMRSH_ADDRMODE_64 :
 ...
 */
uintptr_t
isCacheFileName(OMRPortLibrary* portlib, const char* nameToTest, uintptr_t expectCacheType, const char* optionalExtraID)
{
	J9PortShcVersion versionData;
	intptr_t nameToTestLen = 0;
	intptr_t expectedVersionLen = ((expectCacheType == OMRPORT_SHR_CACHE_TYPE_PERSISTENT) || (OMRPORT_SHR_CACHE_TYPE_SNAPSHOT == expectCacheType)) ? OMRSH_VERSION_STRING_LEN + 1 : OMRSH_VERSION_STRING_LEN;
	...
	if (generation <= OMRSH_GENERATION_29) {
		expectedVersionLen -= OMRSH_VERSTRLEN_INCREASED_SINCEG29;
	}
	if (nameToTest == NULL) {
		return 0;
	}
	...
	if (modLevel < 10) {
		expectedVersionLen -= OMRSH_VERSTRLEN_INCREASED_SINCEJAVA10;
	}
	...
	if (getValuesFromShcFilePrefix(portlib, nameToTest, &versionData) != 0) {
		if (versionData.feature > OMRSH_FEATURE_MAX_VALUE) {
			return 0;
		}
