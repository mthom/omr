...
// #ifdef J9_PROJECT_SPECIFIC // uncomment, normalcy!
// -----------------------------------------------------------------------------
...
TR::X86MemImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
   ...
   if (symbol->isConst())
      ...
      if (cg()->needClassAndMethodPointerRelocations())
         ...
         *(int32_t *)cursor = (int32_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(cg()->comp(), (TR_OpaqueClassBlock*)(uintptr_t)getSourceImmediate());
         if (cg()->comp()->getOption(TR_UseSymbolValidationManager))
            ...
            {
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)getSymbolReference(), getNode() ? (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_ClassAddress, cg()),
                                                                                         __FILE__, __LINE__, getNode());
            }
         }
      }
      ...
      {
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)getSymbolReference(), getNode() ? (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_MethodObject, cg()),
                              __FILE__, __LINE__, getNode());
      }
   ...
   if (getSymbolReference()->getSymbol()->isLabel())
      ...
      switch (getReloKind())
	    ...
	    //if (false)
	       ...
	       char *name = const_cast<char *>(target->externalName(cg()->trMemory()));
	       //cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,reinterpret_cast<uint8_t*>(name), TR_MethodCallAddress, cg()), __FILE__, __LINE__, getNode());
               uint8_t* size = new (cg()->trHeapMemory()) uint8_t(strlen(name)+1);
               //size[0]= 14;                                                              
               cg()->addASHLExternalRelocation( __FILE__, __LINE__, getNode(),reinterpret_cast<void*>(name), size, TR_MethodCallAddress, cursor);
