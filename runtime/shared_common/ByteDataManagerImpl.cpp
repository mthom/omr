...
J9HashTable* 
SH_ByteDataManagerImpl::localHashTableCreate(OMR_VMThread* currentThread, U_32 initialEntries)
{
...
SH_ByteDataManagerImpl::newInstance(OMR_VM* vm, SH_SharedCache* cache_, SH_ByteDataManagerImpl* memForConstructor)
{
...
void
SH_ByteDataManagerImpl::initialize(OMR_VM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
...
IDATA
SH_ByteDataManagerImpl::localInitializePools(OMR_VMThread* currentThread)
{
	Trc_SHR_BDMI_localInitializePools_Entry(currentThread);

	_linkedListImplPool = pool_new(sizeof(SH_ByteDataManagerImpl::BdLinkedListImpl),  0, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_CLASSES, POOL_FOR_PORT(_portlib));
...
void
SH_ByteDataManagerImpl::localTearDownPools(OMR_VMThread* currentThread)
{
 ...
 */
bool 
SH_ByteDataManagerImpl::storeNew(OMR_VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
	...
	if (itemInCache->dataType == TYPE_BYTE_DATA) {
		...
		const OMRUTF8* key = (const OMRUTF8*)BDWTOKEN(bdw);
		UDATA type = BDWTYPE(bdw);

		if (type <= OMRSHR_DATA_TYPE_MAX) {
			_indexedBytesByType[type] += ITEMDATALEN(itemInCache);
			_numIndexedBytesByType[type] += 1;
		} else {
			/* Unidentified types */
			_indexedBytesByType[OMRSHR_DATA_TYPE_UNKNOWN] += ITEMDATALEN(itemInCache);
			_numIndexedBytesByType[OMRSHR_DATA_TYPE_UNKNOWN] += 1;
		}
 ...
 */
ByteDataWrapper*
SH_ByteDataManagerImpl::findSingleEntry(OMR_VMThread* currentThread, const char* key, UDATA keylen, UDATA dataType, U_16 jvmID, UDATA* dataLen)
{
 ...
 */
void
SH_ByteDataManagerImpl::markAllStaleForKey(OMR_VMThread* currentThread, const char* key, UDATA keylen)
{
	BdLinkedListImpl *found, *walk;
...
SH_ByteDataManagerImpl::setDescriptorFields(const ByteDataWrapper* wrapper, OMRSharedDataDescriptor* descriptor)
{
	...
	if (wrapper->privateOwnerID) {
		descriptor->flags |= OMRSHRDATA_IS_PRIVATE;
		if (wrapper->privateOwnerID != _cache->getCompositeCacheAPI()->getJVMID()) {
			descriptor->flags |= OMRSHRDATA_PRIVATE_TO_DIFFERENT_JVM;
		}
	}
} 
...
SH_ByteDataManagerImpl::releasePrivateEntry(OMR_VMThread* currentThread, const OMRSharedDataDescriptor* data)
{
	...
	if (!data || (data->flags & OMRSHRDATA_PRIVATE_TO_DIFFERENT_JVM) || !(data->flags & OMRSHRDATA_IS_PRIVATE) || (data->flags & OMRSHRDATA_USE_READWRITE)) {
 ...
 * OMRSHRDATA_PRIVATE_TO_DIFFERENT_JVM flag set.
 * 
 * @param[in] currentThread  The current thread
...
SH_ByteDataManagerImpl::acquirePrivateEntry(OMR_VMThread* currentThread, const OMRSharedDataDescriptor* data)
{
	...
	if (!data || !(data->flags & OMRSHRDATA_PRIVATE_TO_DIFFERENT_JVM) || (data->flags & OMRSHRDATA_USE_READWRITE)) {
 ...
 */
IDATA
SH_ByteDataManagerImpl::createHintsForCachelet(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, CacheletHints* hints)
{
 ...
 */
IDATA
SH_ByteDataManagerImpl::primeHashtables(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, U_8* hintsData, UDATA dataLength)
{
