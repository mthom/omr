...
#define CACHEMAP_TRACE(verboseLevel, nlsFlags, var1) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1)
#define CACHEMAP_TRACE1(verboseLevel, nlsFlags, var1, p1) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1)
#define CACHEMAP_TRACE2(verboseLevel, nlsFlags, var1, p1, p2) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1, p2)
#define CACHEMAP_TRACE3(verboseLevel, nlsFlags, var1, p1, p2, p3) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1, p2, p3)
#define CACHEMAP_TRACE4(verboseLevel, nlsFlags, var1, p1, p2, p3, p4) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4)

/* Print macros should be used if message should always be printed */
#define CACHEMAP_PRINT(nlsFlags, var1) omrnls_printf(nlsFlags, var1)
#define CACHEMAP_PRINT1(nlsFlags, var1, p1) omrnls_printf(nlsFlags, var1, p1)
#define CACHEMAP_PRINT2(nlsFlags, var1, p1, p2) omrnls_printf(nlsFlags, var1, p1, p2)
#define CACHEMAP_PRINT3(nlsFlags, var1, p1, p2, p3) omrnls_printf(nlsFlags, var1, p1, p2, p3)
#define CACHEMAP_PRINT4(nlsFlags, var1, p1, p2, p3, p4) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4)
#define CACHEMAP_PRINT5(nlsFlags, var1, p1, p2, p3, p4, p5) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4, p5)
#define CACHEMAP_PRINT6(nlsFlags, var1, p1, p2, p3, p4, p5, p6) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4, p5, p6)
#define CACHEMAP_PRINT7(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7)
#define CACHEMAP_PRINT8(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7, p8) omrnls_printf(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7, p8)

#define CACHEMAP_FMTPRINT1(nlsFlags, var1, p1) omrnls_printf(nlsFlags, var1, 1,' ',p1)

static char* formatAttachedDataString(OMR_VMThread* currentThread, U_8 *attachedData, UDATA attachedDataLength, char *attachedDataStringBuffer, UDATA bufferLength);
...
#define OMRSHR_DEFAULT_CACHELET_SIZE (1024 * 1024)
#define OMRSHR_NESTED_CACHE_TEMP_NAME "nested_temp"

/* All flags should be false. Don't EVER update the cache when set. */
#define RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE (OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES | OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)

#define RUNTIME_FLAGS_PREVENT_AOT_DATA_UPDATE (OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES | OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)

#define RUNTIME_FLAGS_PREVENT_JIT_DATA_UPDATE (OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES | OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)
 ...
 */
IDATA
SH_CacheMap::enterLocalMutex(OMR_VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
	if (_isAssertEnabled) {
 ...
 */
IDATA
SH_CacheMap::exitLocalMutex(OMR_VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
	if (_isAssertEnabled) {
 ...
 */
IDATA
SH_CacheMap::enterReentrantLocalMutex(OMR_VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
	IDATA rc = 0;
 ...
 */
IDATA
SH_CacheMap::exitReentrantLocalMutex(OMR_VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
	IDATA rc = 0;
...
IDATA
SH_CacheMap::enterRefreshMutex(OMR_VMThread* currentThread, const char* caller)
{
	IDATA rc;
...
IDATA
SH_CacheMap::exitRefreshMutex(OMR_VMThread* currentThread, const char* caller)
{
	IDATA rc;
 ...
 */
void
SH_CacheMap::dontNeedMetadata(OMR_VMThread* currentThread) 
{
 ...
 */
SH_CacheMapStats*
SH_CacheMap::newInstanceForStats(OMR_VM* vm, SH_CacheMap* memForConstructor, const char* cacheName)
{
	SH_CacheMap* newCacheMap = memForConstructor;
...
SH_CacheMap::initialize(OMR_VM* vm, OMRSharedClassConfig* sharedClassConfig, BlockPtr memForConstructor, const char* cacheName, I_32 cacheTypeRequired, bool startupForStats)
{
	...
	_portlib = vm->_runtime->_portLibrary;
	...
	//	_cpm = SH_ClasspathManagerImpl2::newInstance(vm, this, _tsm, (SH_ClasspathManagerImpl2*)(allocPtr += SH_TimestampManagerImpl::getRequiredConstrBytes()));
		...
		_scm = SH_ScopeManagerImpl::newInstance(vm, this, (SH_ScopeManagerImpl*)(allocPtr += SH_CompositeCacheImpl::getRequiredConstrBytesWithCommonInfo(false, startupForStats))); //SH_ClasspathManagerImpl2::getRequiredConstrBytes()));
	...
	_cmm = SH_CompiledMethodManagerImpl::newInstance(vm, this, (SH_CompiledMethodManagerImpl*)(allocPtr += SH_CompositeCacheImpl::getRequiredConstrBytesWithCommonInfo(false, startupForStats)));
	...
	_adm = SH_AttachedDataManagerImpl::newInstance(vm, this, (SH_AttachedDataManagerImpl*)(allocPtr += SH_CompiledMethodManagerImpl::getRequiredConstrBytes()));
											       //SH_ByteDataManagerImpl::getRequiredConstrBytes()));
 ...
 */
void
SH_CacheMap::cleanup(OMR_VMThread* currentThread)
{
...
//UDATA
//SH_CacheMap::sanityWalkROMClassSegment(OMR_VMThread* currentThread, SH_CompositeCacheImpl* cache)
//{
...
//			CACHEMAP_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_ROMCLASS, walk);
...
SH_CacheMap::startup(OMR_VMThread* currentThread, J9SharedClassPreinitConfig* piconfig, const char* rootName, const char* cacheDirName, UDATA cacheDirPerm, BlockPtr cacheMemoryUT, bool* cacheHasIntegrity)
{
	...
	if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_READONLY) {
...
#if !defined(OMRSHR_CACHELET_SUPPORT)
		if (rc == CC_STARTUP_OK) {
			/* When OMRSHR_CACHELET_SUPPORT is defined there are no J9ROMClasses
...
//			}
		...
		if (rc == CC_STARTUP_OK) {
			...
			if (_ccHead->enterWriteMutex(currentThread, false, fnName) == 0) {
				...
				if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && !_ccHead->isRunningReadOnly()) {
				 ...
				 */
				if ((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED))
				...
				CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_ENTER_WRITE_MUTEX_STARTUP);
		...
		if (0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_STATS) && (false == _ccHead->isRunningReadOnly())) {
			...
			if ((rc == CC_STARTUP_CORRUPT) || (rc == CC_STARTUP_RESET) || (rc == CC_STARTUP_SOFT_RESET)) {
				...
				deleteRC = _ccHead->deleteCache(currentThread, (rc == CC_STARTUP_SOFT_RESET) && !(_verboseFlags & OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE));
	...
	if (_ccHead->isRunningReadOnly()) {
		*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_READONLY;
		/* If running read-only, treat the cache as full */
		_ccHead->markReadOnlyCacheFull();
	}

//	if (!initializeROMSegmentList(currentThread)) {
//		CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_CREATE_ROMIMAGE);
...
//		}
//		/* CMVC 160728:
//		 * Calling 'updateROMSegmentList()' here when OMRSHR_CACHELET_SUPPORT is defined caused
...
//		 *
//		 * When OMRSHR_CACHELET_SUPPORT is defined, and we are creating a new cache, it should be noted
...
//J9MemorySegment* 
//SH_CacheMap::addNewROMImageSegment(OMR_VMThread* currentThread, U_8* segmentBase, U_8* segmentEnd)
//{
...
//J9MemorySegment* 
//SH_CacheMap::createNewSegment(OMR_VMThread* currentThread, UDATA type, J9MemorySegmentList* segmentList, 
 ...
 */
//void
//SH_CacheMap::updateROMSegmentList(OMR_VMThread* currentThread, bool hasClassSegmentMutex)
//{
 ...
 */
//void
//SH_CacheMap::updateROMSegmentListForCache(OMR_VMThread* currentThread, SH_CompositeCacheImpl* forCache)
//{
//	OMR_VM* vm = currentThread->javaVM;
...
//				CACHEMAP_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_ROMCLASS, currentROMClass);
 ...
 */
//UDATA 
//SH_CacheMap::initializeROMSegmentList(OMR_VMThread* currentThread)
//{
//	OMR_VM* vm = currentThread->javaVM;
//	UDATA result = 1;
...
IDATA
SH_CacheMap::startManager(OMR_VMThread* currentThread, SH_Manager* manager)
{
...
IDATA 
SH_CacheMap::getAndStartManagerForType(OMR_VMThread* currentThread, UDATA dataType, SH_Manager** startedManager)
{
	SH_Manager* manager;
	IDATA result = 0;
...
IDATA
SH_CacheMap::readCacheUpdates(OMR_VMThread* currentThread)
{
	IDATA itemsRead = 0;
 ...
 */
IDATA
SH_CacheMap::readCache(OMR_VMThread* currentThread, SH_CompositeCacheImpl* cache, IDATA expectedUpdates, bool startupForStats)
{
	ShcItem* it = NULL;
	IDATA result = 0;
	...
	do {
		...
		if (it) {
			...
			if ((itemType <= TYPE_UNINITIALIZED) || (itemType > MAX_DATA_TYPES)) {
				CACHEMAP_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_DATA, it);
				...
				if (rc == -1) {
					...
					if (manager->storeNew(currentThread, it, cache)) {
						...
						CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_HASHTABLE_ADD_FAILURE);
...
IDATA
SH_CacheMap::checkForCrash(OMR_VMThread* currentThread, bool hasClassSegmentMutex)
{
	IDATA rc = 0;
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
	...
	if (_ccHead->crashDetected(&_localCrashCntr)) {
		CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_INFO, J9NLS_SHRC_CM_UNEXPECTED_TERMINATION_DETECTED);
 ...
 */
IDATA 
SH_CacheMap::refreshHashtables(OMR_VMThread* currentThread, bool hasClassSegmentMutex)
{
	IDATA itemsRead = 0;
...
IDATA
SH_CacheMap::resetAllManagers(OMR_VMThread* currentThread)
{		
	SH_Manager* walkManager;
	SH_Managers::ManagerWalkState state;
...
void
SH_CacheMap::updateAllManagersWithNewCacheArea(OMR_VMThread* currentThread, SH_CompositeCacheImpl* newCacheArea)
{
	SH_Manager* walkManager;
	SH_Managers::ManagerWalkState state;
...
SH_CacheMap::getCacheAreaForDataType(OMR_VMThread* currentThread, UDATA dataType, UDATA dataLength)
{
	...
	if (dataLength > OMRSHR_DEFAULT_CACHELET_SIZE) {
		return NULL;
	}
...
//ClasspathWrapper* 
//SH_CacheMap::updateClasspathInfo(OMR_VMThread* currentThread, ClasspathItem* cp, I_16 cpeIndex, const J9UTF8* partition, const J9UTF8** cachedPartition, const J9UTF8* modContext, const J9UTF8** cachedModContext, bool haveWriteMutex) {
...
const OMRUTF8*
SH_CacheMap::addScopeToCache(OMR_VMThread* currentThread, const OMRUTF8* scope) 
{
	const OMRUTF8* result = NULL;
	ShcItem item;
	ShcItem* itemPtr = &item;
	ShcItem* itemInCache = NULL;
	U_32 totalSizeNeeded = OMRUTF8_LENGTH(scope) + sizeof(struct OMRUTF8);
	...
	Trc_SHR_CM_addScopeToCache_Entry(currentThread, OMRUTF8_LENGTH(scope), OMRUTF8_DATA(scope));
	...
	if (localSCM->storeNew(currentThread, itemInCache, cacheForAllocate)) {
		result = (const OMRUTF8*)ITEMDATA(itemInCache);
	}
...
/* THREADING: MUST be called with cache write mutex held */
//ClasspathWrapper* 
//SH_CacheMap::addClasspathToCache(OMR_VMThread* currentThread, ClasspathItem* obj) 
//{
 ...
 */
IDATA 
SH_CacheMap::runEntryPointChecks(OMR_VMThread* currentThread, void* isAddressInCache, const char** p_subcstr)
{
	...
	if (_ccHead->isCacheCorrupt()) {
		reportCorruptCache(currentThread);
		if (NULL != p_subcstr) {
			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_CACHE_CORRUPT, "cache is corrupt");
		}
	...
	if (!_ccHead->isRunningReadOnly()) {
		if (_ccHead->hasWriteMutex(currentThread)) {
		  ...
		  rc = 0; // we no longer have class segments in OMR
			if(rc < 0) {
...
//		/* Error reported */
//		if (NULL != p_subcstr) {
//			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),  J9NLS_SHRC_CM_CACHE_REFRESH_FAILED, "cache refresh failed");
//		}
...
//		if (_ccHead->hasWriteMutex(currentThread)) {
//			hasMutex = true;
//		} else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND)) {
 ...
 */
//const J9ROMClass*
//SH_CacheMap::allocateROMClassOnly(OMR_VMThread* currentThread, U_32 sizeToAlloc, U_16 classnameLength, const char* classnameData, ClasspathWrapper* cpw, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, IDATA callerHelperID, bool modifiedNoContext, void * &newItemInCache, void * &cacheAreaForAllocate)
//{
//	const J9ROMClass* result = NULL;
//	bool fullFlagSet = OMR_ARE_ANY_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
 ...
 */
//void
//SH_CacheMap::tokenStoreStaleCheckAndMark(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData, ClasspathWrapper* cpw, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, UDATA callerHelperID)
//{
//	/* If we are storing using a Token and there is already a class of the same name stored under that token, mark the original one stale */
 ...
 */
//const J9ROMClass*
//SH_CacheMap::allocateFromCache(OMR_VMThread* currentThread, U_32 sizeToAlloc, U_32 wrapperSize, U_16 wrapperType, void * &newItemInCache, void * &cacheAreaForAllocate)
//{
//	ShcItem item;
//	ShcItem* itemPtr = &item;
//	ShcItem* itemInCache = NULL;
 ...
 */
//IDATA
//SH_CacheMap::allocateClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData, const J9RomClassRequirements * sizes, J9SharedRomClassPieces * pieces)
//{
 ...
 */
//void 
//SH_CacheMap::rollbackClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData)
//{
 ...
 */
//void 
//SH_CacheMap::commitClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData)
//{
//	this->_ccHead->commitClassDebugData(currentThread, classnameLength, classnameData);
//}
//
void
SH_CacheMap::updateLineNumberContentInfo(OMR_VMThread* currentThread)
{
        OMR_VM * vm = currentThread->_vm;

	if (false == this->_cc->getIsNoLineNumberContentEnabled()) {
	  // OMR_VM has no 'requiredDebugAttributes' field.
 ...
 */
//IDATA
//SH_CacheMap::commitROMClass(OMR_VMThread* currentThread, ShcItem* itemInCache, SH_CompositeCacheImpl* cacheAreaForAllocate, ClasspathWrapper* cpw, I_16 cpeIndex, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, BlockPtr romClassBuffer, bool commitOutOfLineData)
//{
//	IDATA retval = 0;
...
//
//	if ((true == useWriteHash) && (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)) {
...
//	if ((true == useWriteHash) && (cpw != NULL) && (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)) {
//		if (classNameHash == 0) {
//			classNameHash = currentThread->javaVM->internalVMFunctions->computeHashForUTF8(J9UTF8_DATA(romClassName), J9UTF8_LENGTH(romClassName));
//		}
 ...
 */
//IDATA
//SH_CacheMap::commitOrphanROMClass(OMR_VMThread* currentThread, ShcItem* itemInCache, SH_CompositeCacheImpl* cacheAreaForAllocate, ClasspathWrapper* cpw, BlockPtr romClassBuffer)
//{
//	IDATA retval = 0;
//	bool storeResult = false;
//	J9UTF8* romClassName = J9ROMCLASS_CLASSNAME(((J9ROMClass *) romClassBuffer));
 ...
 */
//IDATA
//SH_CacheMap::commitMetaDataROMClassIfRequired(OMR_VMThread* currentThread, ClasspathWrapper* cpw, I_16 cpeIndex, IDATA helperID, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, J9ROMClass * romclass)
//{
//	IDATA retval = 0;
...
//				_writeHashMaxWaitMicros = _writeHashSavedMaxWaitMicros;
//				if (classNameHash == _writeHashContendedResetHash) {
//					OMRPORT_ACCESS_FROM_VMC(currentThread);
//					_writeHashContendedResetHash = 0;
 ...
 */
//const J9ROMClass *
//SH_CacheMap::findNextROMClass(OMR_VMThread* currentThread, void * &findNextIterator, void * &firstFound, U_16 classnameLength, const char* classnameData)
//{
//
 ...
 * Locate a shared ROM class in the cache
 *
 * @param [in] currentThread Pointer to OMR_VMThread structure for the current thread
 ...
 */
//const J9ROMClass*
//SH_CacheMap::findROMClass(OMR_VMThread* currentThread, const char* path, ClasspathItem* cp, const J9UTF8* partition, const J9UTF8* modContext, IDATA confirmedEntries, IDATA* foundAtIndex)
//{
//	UDATA rc = 0;
...
//	*/
//	if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && (rc & LOCATE_ROMCLASS_RETURN_DO_TRY_WAIT)) {
...
//		if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && (true == useWriteHash)) {
//			if (hash == 0) {
//				hash = currentThread->javaVM->internalVMFunctions->computeHashForUTF8((U_8*)path, (U_16)pathLen);
//			}
//
...
//#if defined(LINUX)
//				&& OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE)
//#endif
//		) {
...
//		} else
//#if defined(LINUX)
//			if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE))
//#endif
//		{
...
/* THREADING: MUST be protected by cache write mutex - therefore single-threaded within this JVM */
const void*
SH_CacheMap::addROMClassResourceToCache(OMR_VMThread* currentThread, const void* romAddress, SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor, const char** p_subcstr)
{
	...
	if (!cacheAreaForAllocate) {
		...
		/* TODO: In offline mode, should be fatal */
		if (NULL != p_subcstr) {
			const char* tmpstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),J9NLS_SHRC_CM_CANNOT_ALLOC_DATA_SIZE,"no space in cache for %d bytes");
			omrstr_printf((char *)*p_subcstr, VERBOSE_BUFFER_SIZE, tmpstr, dataLength);
		}
	...
	case TYPE_COMPILED_METHOD :
		...
		break;
	case TYPE_ATTACHED_DATA :
		if ((OMRSHR_ATTACHED_DATA_TYPE_JITPROFILE == resourceSubType) || 
			(OMRSHR_ATTACHED_DATA_TYPE_JITHINT == resourceSubType)
		){
	...
	if (itemInCache == NULL) {
		...
		return (void*)OMRSHR_RESOURCE_STORE_FULL;
	}
...
const void*
SH_CacheMap::storeROMClassResource(OMR_VMThread* currentThread, const void* romAddress, SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor, UDATA forceReplace, const char** p_subcstr)
{
	...
	if (!localRRM->permitAccessToResource(currentThread)) {
		if (p_subcstr) {
			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_NO_ACCESS_TO_RESOURCE, "no access to resource");
		}
	...
	if (_ccHead->enterWriteMutex(currentThread, false, fnName) != 0) {
		if (p_subcstr) {
			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_ENTER_WRITE_MUTEX, "enterWriteMutex failed");
		}
	...
	if ((resourceWrapper == (void*)OMRSHR_RESOURCE_STORE_FULL) || (resourceWrapper == (void*)OMRSHR_RESOURCE_STORE_ERROR)) {
...
SH_CacheMap::updateROMClassResource(OMR_VMThread* currentThread, const void* addressInCache, I_32 updateAtOffset, SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor, const J9SharedDataDescriptor* data, bool isUDATA , const char** p_subcstr)
{
	...
	if (!localRRM->permitAccessToResource(currentThread)) {
		...
		return OMRSHR_RESOURCE_STORE_ERROR;
	}
	...
	do {
		...
		if (_ccHead->enterWriteMutex(currentThread, true, fnName) != 0) {
			result = OMRSHR_RESOURCE_STORE_ERROR;
			if (NULL != p_subcstr) {
				*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_ENTER_WRITE_MUTEX, "enterWriteMutex failed");
			}
		...
		if ((resourceWrapper = (localRRM->findResource(currentThread, resourceKey))) != 0) {
			...
			if ((UDATA)updateAtOffset+data->length > dataLength) {
				if (NULL != p_subcstr) {
					const char* tmpcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_DATA_SIZE_LARGER, "data %d larger than available %d");
					omrstr_printf((char *)*p_subcstr, VERBOSE_BUFFER_SIZE, tmpcstr, updateAtOffset+(data->length), dataLength);
				}
			...
			if(NULL != p_subcstr) {
				*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),J9NLS_SHRC_CM_NO_DATA_EXISTS,"no data exists");
			}
 ...
 * @param [out] flags OMRSHR_AOT_METHOD_FLAG_INVALIDATED is set if the compiled method has been invalidated
 *
 ...
 */
const void*
SH_CacheMap::findROMClassResource(OMR_VMThread* currentThread, const void* romAddress, SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor, bool useReadMutex, const char** p_subcstr, UDATA* flags)
{
	const void* result = NULL;
	...
	if ((true == useReadMutex) && (_ccHead->enterReadMutex(currentThread, fnName) != 0)) {
		if (NULL != p_subcstr) {
			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_ENTER_READ_MUTEX, "enterReadMutex failed");
		}
	...
	if ((resourceWrapper = localRRM->findResource(currentThread, resourceKey)) != 0) {
		if (TYPE_INVALIDATED_COMPILED_METHOD != ITEMTYPE(resourceDescriptor->wrapperToItem(resourceWrapper))) {
			...
			if (NULL != flags) {
				*flags |= OMRSHR_AOT_METHOD_FLAG_INVALIDATED;
			}
		}
	}
 ...
 * data is indexed by the methodNameAndSignature's address. The return value is a pointer to the
 * stored data or null in case of an error.
 *
 * @param [in] currentThread Pointer to OMR_VMThread structure for the current thread
 * @param [in] methodNameAndSignature Pointer to the J9ROMMethod structure
 ...
 * @return OMRSHR_RESOURCE_STORE_EXISTS if the method already exists in the cache
 * (will never return this if forceReplace is non-null)
 * @return OMRSHR_RESOURCE_STORE_ERROR if an error occurred storing the method
 * @return OMRSHR_RESOURCE_STORE_FULL if the cache is full
 * @return OMRSHR_RESOURCE_STORE_INVALIDATED if the method already exits in the cache and it has been invalidated
 ...
 */
const U_8*
SH_CacheMap::storeCompiledMethod(OMR_VMThread* currentThread, const MethodNameAndSignature* methodNameAndSignature, const U_8* dataStart, UDATA dataSize, const U_8* codeStart, UDATA codeSize, UDATA forceReplace)
{
	const U_8* result;
	...
	result = (const U_8*)storeROMClassResource(currentThread, methodNameAndSignature, localCMM, &descriptor, forceReplace, NULL);
 ...
 * The stored data is indexed by the methodNameAndSignature's address. The return value is a 
 * pointer to the stored data or null if no matching data exists in the cache.
 *
 * @param [in] currentThread Pointer to OMR_VMThread structure for the current thread
 * @param [in] methodNameAndSignature Pointer to the MethodNameAndSignature structure for which the stored data is required
 * @param [out] flags OMRSHR_AOT_METHOD_FLAG_INVALIDATED is set if the compiled method has been invalidated
 *
 ...
 */
const U_8*
SH_CacheMap::findCompiledMethod(OMR_VMThread* currentThread, const MethodNameAndSignature* methodNameAndSignature, UDATA* flags)
{
	const U_8* result;
	...
	result = (const U_8*)findROMClassResource(currentThread, methodNameAndSignature, localCMM, &descriptor, true, NULL, flags);
#if !defined(J9ZOS390) && !defined(AIXPPC)
	if (_metadataReleased
#if defined(LINUX)
			&& OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE)
...
//			J9ROMClass* romClass = vmFunctions->findROMClassFromPC(currentThread, (UDATA)methodNameAndSignature, &loader);
//			if (NULL != romClass) {
//				J9UTF8* romClassName = J9ROMCLASS_CLASSNAME(romClass);
//				J9UTF8* methodNameAndSignatureName = J9ROMMETHOD_NAME(methodNameAndSignature);
//				J9UTF8* methodNameAndSignatureSig = J9ROMMETHOD_SIGNATURE(methodNameAndSignature);
//				Trc_SHR_CM_findCompiledMethod_metadataAccess(
//						currentThread,
//						J9UTF8_LENGTH(romClassName),
//						J9UTF8_DATA(romClassName),
//						J9UTF8_LENGTH(methodNameAndSignatureName),
//						J9UTF8_DATA(methodNameAndSignatureName),
//						J9UTF8_LENGTH(methodNameAndSignatureSig),
//						J9UTF8_DATA(methodNameAndSignatureSig),
//						result
//				);
//			}
		}
 ...
 */
void
SH_CacheMap::updateAccessedShrCacheMetadataBounds(OMR_VMThread* currentThread, uintptr_t const * metadataAddress)
{
...
* @return OMRSHR_RESOURCE_STORE_EXISTS if the attached data already exists in the cache
* (will never return this if forceReplace is non-null)
...
SH_CacheMap::storeAttachedData(OMR_VMThread* currentThread, const void* addressInCache, const J9SharedDataDescriptor *data, UDATA forceReplace)
{
	...
	if (localVerboseFlags  & OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA) {
		...
		if (OMRSHR_RESOURCE_MAX_ERROR_VALUE < (UDATA)result) {
			result = 0;
		}

		if(addressInCache && _ccHead->isAddressInCache(addressInCache)) {
//			J9ClassLoader* loader;
//			J9InternalVMFunctions *vmFunctions = currentThread->_vm->internalVMFunctions;
//			J9ROMClass* romClass = vmFunctions->findROMClassFromPC(currentThread, (UDATA)addressInCache, &loader);
//			J9UTF8* methodName = J9ROMMETHOD_GET_NAME(romClass, (MethodNameAndSignature *)addressInCache);
//			J9UTF8* methodSig = J9ROMMETHOD_GET_SIGNATURE(romClass, (MethodNameAndSignature *)addressInCache);
// 			J9UTF8* className = J9ROMCLASS_CLASSNAME(romClass);
//
//			if ( 0 == result ) {
//				if (OMRSHR_ATTACHED_DATA_TYPE_JITHINT == data->type) {
...
*/
UDATA
SH_CacheMap::updateAttachedUDATA(OMR_VMThread* currentThread, const void* addressInCache, UDATA type, I_32 updateAtOffset, UDATA value)
{
	UDATA result = 0;
...
* @return OMRSHR_RESOURCE_STORE_ERROR if the data buffer in the parameter is not sufficient to store the data found
* @return OMRSHR_RESOURCE_BUFFER_ALLOC_FAILED if failed to allocate memory for data buffer
* @return OMRSHR_RESOURCE_TOO_MANY_UPDATES if not able to read consistent data when running read-only
*
*/
const U_8*
...
//			J9UTF8* className = J9ROMCLASS_CLASSNAME(romClass);
//
//			if (( NULL == result) || (OMRSHR_RESOURCE_MAX_ERROR_VALUE >= (UDATA)result)) {
	...
	if (NULL != result) {
		...
		if (NULL != data->address) {
			if (data->length < dataLength) {
				result = (U_8 *)OMRSHR_RESOURCE_STORE_ERROR;
				if (NULL != p_subcstr) {
					const char *tmpcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_DATA_SIZE_LARGER, "data %d larger than available %d");
					omrstr_printf((char *)*p_subcstr, VERBOSE_BUFFER_SIZE, tmpcstr, dataLength, data->length);
				}
				goto _exitWithError;
			}
		} else {
			data->address = (U_8 *)omrmem_allocate_memory(dataLength, OMRMEM_CATEGORY_CLASSES);
			if (NULL == data->address) {
				result = (const U_8 *)OMRSHR_RESOURCE_BUFFER_ALLOC_FAILED;
				if (NULL != p_subcstr) {
					const char *tmpcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_MEMORY_ALLOC_FAILED, "memory allocation of %d bytes failed");
					omrstr_printf((char *)*p_subcstr, VERBOSE_BUFFER_SIZE, tmpcstr, dataLength);
				}
				goto _exit;
			}
			...
			do {
				...
				if (initialUpdateCount != finalUpdateCount) {
					if((++updateCountTryCount) > FIND_ATTACHED_DATA_RETRY_COUNT) {
						result = (U_8 *)OMRSHR_RESOURCE_TOO_MANY_UPDATES;
						if (NULL != p_subcstr) {
							*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_TOO_MANY_UPDATES, "too many updates while reading");
						}
						goto _exitWithError;
					} else {
						continue;
		...
		if (NULL != p_subcstr) {
			*p_subcstr = omrnls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_CACHE_FIND_NOTFOUND, "no data in cache");
		}
	}

_exitWithError:
	if (true == bufferAllocated) {
		omrmem_free_memory(data->address);
		data->address = NULL;
	}
_exit:
	_ccHead->exitReadMutex(currentThread, fnName);
...
//	ShcItem* itemPtr = &item;
//	ShcItem* itemInCache = NULL;
//	UDATA dataNotIndexed = (data->flags & OMRSHRDATA_NOT_INDEXED);
//	UDATA dataReadWrite = (data->flags & OMRSHRDATA_USE_READWRITE);
//	UDATA dataIsPrivate = (data->flags & OMRSHRDATA_IS_PRIVATE);
//	BlockPtr memToSet;
...
//
//	if (OMR_ARE_ANY_BITS_SET(*_runtimeFlags, RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE)) {
//		if (OMR_ARE_ALL_BITS_SET(data->flags, OMRSHRDATA_USE_READWRITE)) {
...
//
//	if (!dataReadWrite) {
//		if (data->type != OMRSHR_DATA_TYPE_CACHELET) {
...
//#if defined(OMRSHR_CACHELETS_SAVE_READWRITE_AREA)
//		if (!forceCache) {
//			cacheForAllocate = getCacheAreaForDataType(currentThread, wrapperType, _ccHead->getBytesRequiredForItemWithAlign(itemPtr, SHC_WORDALIGN, sizeof(ByteDataWrapper)));
//			if (!cacheForAllocate) {
//				/* This may indicate size required is bigger than the cachelet size. */
...
//		return NULL;
//	}
//	if (data->flags & OMRSHRDATA_ALLOCATE_ZEROD_MEMORY) {
//		memset(memToSet, 0, data->length);
//	} else {
//		memcpy(memToSet, data->address, data->length);
//	}
//
//	/* storeNew is effectively a no-op for data marked as OMRSHRDATA_NOT_INDEXED */
//	if (localWriteWithoutMetadata) {
//		result = memToSet;
...
//
//	if ((NULL != bdwInCache) && (OMRSHR_DATA_TYPE_AOTHEADER == bdwInCache->dataType)) {
 ...
 * If OMRSHRDATA_SINGLE_STORE_FOR_KEY_TYPE is set, one store for that key/dataType combination is allowed. Subsequent stores will simply return the existing data.
 * If OMRSHRDATA_SINGLE_STORE_FOR_KEY_TYPE is not set, the following is true:
 ...
 *   OMRSHRDATA_IS_PRIVATE - the data is private to this JVM
 *   OMRSHRDATA_ALLOCATE_ZEROD_MEMORY - allocate zero'd space in the cache to be written into later
 *      bear in mind that this should be either private memory which doesn't require locking, or read/write memory
 *   OMRSHRDATA_USE_READWRITE - allocate into the cache read/write area
 *   OMRSHRDATA_NOT_INDEXED - add the data, but don't index it (saves cache space)
 *      data must therefore be referenced by other data as it can never be retrieved by findSharedData
 *   OMRSHRDATA_SINGLE_STORE_FOR_KEY_TYPE - only allow one store for a given key/type combination
...
SH_CacheMap::storeSharedData(OMR_VMThread* currentThread, const char* key, UDATA keylen, const J9SharedDataDescriptor* data)
{
	...
	OMRUTF8* utfKeyStruct = NULL;
	UDATA dataNotIndexed = (data != NULL) ? (data->flags & OMRSHRDATA_NOT_INDEXED) : 0;
	...
	if (_runningNested && (data != NULL) && (data->flags & OMRSHRDATA_USE_READWRITE)) {
		dataNotIndexed = true;
	}
	...
	if (!dataNotIndexed) {
		if (data != NULL) {
//			bdwInCache = localBDM->findSingleEntry(currentThread, key, keylen, data->type, ((data->flags & OMRSHRDATA_IS_PRIVATE) ? _ccHead->getJVMID() : 0), &foundDatalen);
			if ((OMRSHR_DATA_TYPE_AOTHEADER == data->type)
...
//					}
//				} else {
//					if (data->flags & OMRSHRDATA_SINGLE_STORE_FOR_KEY_TYPE) {
...
//		}
	...
	if ((data != NULL) && (data->length > 0) && ((data->address != NULL) || (data->flags & OMRSHRDATA_ALLOCATE_ZEROD_MEMORY))) {
		const OMRUTF8* tokenKey = NULL;	

		if (!dataNotIndexed) {
			/* Create OMRUTF8 struct as key */
			if (keylen >= (STACK_STRINGBUF_SIZE - sizeof(OMRUTF8))) {
				if (!(utfKeyPtr = (char*)omrmem_allocate_memory((keylen * sizeof(U_8)) + sizeof(ShcItem), OMRMEM_CATEGORY_CLASSES))) {
					Trc_SHR_CM_storeSharedData_NoMem(currentThread);
					result = NULL;
					goto _done;
				}
			}
			utfKeyStruct = (OMRUTF8*)utfKeyPtr;
			OMRUTF8_SET_LENGTH(utfKeyStruct, (U_16)keylen);
			strncpy((char*)OMRUTF8_DATA(utfKeyStruct), (char*)key, keylen);
 ...
 * OMRSHRDATA_IS_PRIVATE flag set and the OMRSHRDATA_PRIVATE_TO_DIFFERENT_JVM flag not set.
...
//UDATA 
//SH_CacheMap::getJavacoreData(OMR_VM *vm, J9SharedClassJavacoreDataDescriptor* descriptor)
//{
...
//		UDATA type;
//		for (type = 0; type <= OMRSHR_ATTACHED_DATA_TYPE_MAX; type++) {
//			switch (type) {
//			case OMRSHR_ATTACHED_DATA_TYPE_UNKNOWN:
//				break;
//			case OMRSHR_ATTACHED_DATA_TYPE_JITPROFILE:
//				descriptor->jitProfileDataBytes += _adm->getDataBytesForType(type);
//				descriptor->numJitProfiles += _adm->getNumOfType(type);
//				break;
//			case OMRSHR_ATTACHED_DATA_TYPE_JITHINT:
...
//IDATA 
//SH_CacheMap::markStale(OMR_VMThread* currentThread, ClasspathEntryItem* cpei, bool hasWriteMutex) 
//{
//	UDATA unused = 0;
//	ShcItem* it = NULL;
//	IDATA retryCount = 0;
...
//
//			CACHEMAP_TRACE3(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_MARK_STALE, cpeiPathLen, cpeiPath, numMarked);
//
//			if (!hasWriteMutex) {
//				_ccHead->exitWriteMutex(currentThread, fnName);
//			} else {
...
//		CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_MARKSTALE_ATTEMPTS);
...
//void 
//SH_CacheMap::markItemStale(OMR_VMThread* currentThread, const ShcItem* item, bool isCacheLocked)
//{
//	if (_ccHead->isRunningReadOnly()) {
//		return;
//	}
 ...
 */
//void 
//SH_CacheMap::markItemStaleCheckMutex(OMR_VMThread* currentThread, const ShcItem* item, bool isCacheLocked)
//{
...
void 
SH_CacheMap::destroy(OMR_VMThread* currentThread)
{
...
void
SH_CacheMap::reportCorruptCache(OMR_VMThread* currentThread)
{
	...
	if (hasRefreshMutex || enteredRefreshMutex) {
		if (!_cacheCorruptReported) {
			...
			CACHEMAP_TRACE3(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_WARN_CORRUPT_CACHE_V2, _cacheName, corruptionCode, corruptValue);
			/* Reset the writeHash field in the cache as no more updates can occur */
			if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION) && (false == _ccHead->isRunningReadOnly())) {
				_ccHead->setWriteHash(currentThread, 0);
			}
			_cacheCorruptReported = true;
			*_runtimeFlags |= (OMRSHR_RUNTIMEFLAG_DENY_CACHE_ACCESS | OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES);
		}
...
/* THREADING: Can be called multi-threaded */
void
SH_CacheMap::resetCorruptState(OMR_VMThread* currentThread, UDATA hasRefreshMutex)
{
...
/* THREADING: Only ever single-threaded */
IDATA
SH_CacheMap::printAllCacheStats(OMR_VMThread* currentThread, UDATA showFlags, SH_CompositeCacheImpl* cache, U_32* staleBytes)
{
...
//	ClasspathWrapper* cpw = NULL;
	ShcItem* it;
	bool showAllStaleFlag = OMR_ARE_ALL_BITS_SET(showFlags, PRINTSTATS_SHOW_ALL_STALE);
	bool isStale = false;
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
...
//					OMRUTF8* romClassName = J9ROMCLASS_CLASSNAME(romClass);
//					CACHEMAP_PRINT5(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ORPHAN_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(romClassName), OMRUTF8_DATA(romClassName), romClass);
//                              }
				break;
			...
			case TYPE_BYTE_DATA :
			{
				...
				OMRUTF8* pointer =  (OMRUTF8*)BDWTOKEN(bdw);
				UDATA type = (UDATA)BDWTYPE(bdw);

				if (OMRSHR_DATA_TYPE_ZIPCACHE == type) {
					if ((PRINTSTATS_SHOW_ZIPCACHE == (showFlags & PRINTSTATS_SHOW_ZIPCACHE))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_ZIPCACHE_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_JITHINT == type) {
					if ((PRINTSTATS_SHOW_JITHINT == (showFlags & PRINTSTATS_SHOW_JITHINT))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_JITHINT_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_JCL == type) {
					if ((PRINTSTATS_SHOW_JCL == (showFlags & PRINTSTATS_SHOW_JCL))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_JCL_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_AOTCLASSCHAIN == type) {
					if ((PRINTSTATS_SHOW_AOTCH == (showFlags & PRINTSTATS_SHOW_AOTCH))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTCH_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_AOTTHUNK == type) {
					if ((PRINTSTATS_SHOW_AOTTHUNK == (showFlags & PRINTSTATS_SHOW_AOTTHUNK))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTTHUNK_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_AOTHEADER == type) {
					if ((PRINTSTATS_SHOW_AOTDATA == (showFlags & PRINTSTATS_SHOW_AOTDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTDATA_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_HELPER == type) {
					if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_HELPER_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_POOL == type) {
					if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_POOL_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_VM == type) {
					if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_VM_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_ROMSTRING == type) {
					if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_ROMSTRING_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
				} else if (OMRSHR_DATA_TYPE_UNUSED1 == type) {
					if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
						|| (isStale && showAllStaleFlag)
					) {
						CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_UNUSED1_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
						if (isStale) {
							CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
						}
					}
					...
					CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_UNKNOWN_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(pointer), OMRUTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
					if (isStale) {
						CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
					}
				}
...
//							J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(romClassName), OMRUTF8_DATA(romClassName), RCWROMCLASS(rcw));
//					if (isStale) {
//						CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
//					}
//					omrtty_printf("\n");
...
//						const OMRUTF8* partition = (const OMRUTF8*)RCWPARTITION(srcw);
//						const OMRUTF8* modContext = (const OMRUTF8*)RCWMODCONTEXT(srcw);
//
//						if (partition && !modContext) {
//							CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_PARTITION_DISPLAY, OMRUTF8_LENGTH(partition), OMRUTF8_DATA(partition));
//						} else
//						if (modContext && !partition) {
//							CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_MODCONTEXT_DISPLAY, OMRUTF8_LENGTH(modContext), OMRUTF8_DATA(modContext));
//						} else
//						if (modContext && partition) {
//							CACHEMAP_PRINT4(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_PARTITIONINMOD_DISPLAY, OMRUTF8_LENGTH(partition), OMRUTF8_DATA(partition), OMRUTF8_LENGTH(modContext), OMRUTF8_DATA(modContext));
//						}
//					}
//
//					if (showFlags & PRINTSTATS_SHOW_ROMMETHOD) {
//						MethodNameAndSignature *methodNameAndSignature;
//						methodNameAndSignature = J9ROMCLASS_ROMMETHODS(romClass);
//						for (i = 0; i < romClass->methodNameAndSignatureCount; i++) {
//							OMRUTF8* methodNameAndSignatureName;
//							OMRUTF8* methodNameAndSignatureSig;
//
//							if (!methodNameAndSignature) {
//								break;
//							}
//						
//							methodNameAndSignatureName = J9ROMMETHOD_GET_NAME(romClass, methodNameAndSignature);
//							methodNameAndSignatureSig = J9ROMMETHOD_GET_SIGNATURE(romClass, methodNameAndSignature);
//							if (methodNameAndSignatureName && methodNameAndSignatureSig) {
//								CACHEMAP_PRINT5(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMMETHOD_DISPLAY,
//										OMRUTF8_LENGTH(methodNameAndSignatureName), OMRUTF8_DATA(methodNameAndSignatureName), OMRUTF8_LENGTH(methodNameAndSignatureSig), OMRUTF8_DATA(methodNameAndSignatureSig), methodNameAndSignature);
//							}
//
//							methodNameAndSignature = nextROMMethod(methodNameAndSignature);
//						}
//					}
//				}
//				break;
...
//				break;
			...
			case TYPE_COMPILED_METHOD :
				if ((OMR_ARE_ALL_BITS_SET(showFlags, PRINTSTATS_SHOW_AOT) && (TYPE_COMPILED_METHOD == ITEMTYPE(it)))
					|| (OMR_ARE_ALL_BITS_SET(showFlags, PRINTSTATS_SHOW_INVALIDATEDAOT) && (TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)))
					|| (showAllStaleFlag && isStale)
				) {
					CompiledMethodWrapper* cmw = (CompiledMethodWrapper*)ITEMDATA(it);
					MethodNameAndSignature* methodNameAndSignature = (MethodNameAndSignature*)CMWROMMETHOD(cmw);
//					J9ClassLoader* loader;
//					OMRUTF8* romClassName = NULL;
//					J9ROMClass* romClass = vmFunctions->findROMClassFromPC(currentThread, (UDATA)methodNameAndSignature, &loader);
// 					OMRUTF8* methodNameAndSignatureName = J9ROMMETHOD_GET_NAME(romClass, methodNameAndSignature);
//					OMRUTF8* methodNameAndSignatureSig = J9ROMMETHOD_GET_SIGNATURE(romClass, methodNameAndSignature);
//					
// 					if (romClass) {
//						romClassName = J9ROMCLASS_CLASSNAME(romClass);
// 					}
//
// 					if (methodNameAndSignatureName) {
//						CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
//								J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(methodNameAndSignatureName), OMRUTF8_DATA(methodNameAndSignatureName));
// 					}
//
//					if (methodNameAndSignatureSig) {
//						CACHEMAP_PRINT3((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
//								J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY_SIG_ADDR, OMRUTF8_LENGTH(methodNameAndSignatureSig), OMRUTF8_DATA(methodNameAndSignatureSig), methodNameAndSignature);
//					}
//
//					if (isStale) {
//						omrtty_printf(" ");
...
//								J9NLS_SHRC_CM_PRINTSTATS_AOT_FROM_ROMCLASS, OMRUTF8_LENGTH(romClassName), OMRUTF8_DATA(romClassName), romClass);
//					}
				}
				break;
			...
			case TYPE_ATTACHED_DATA :
				if (0 != (showFlags & (PRINTSTATS_SHOW_JITPROFILE | PRINTSTATS_SHOW_JITHINT))
					...
					if ((OMRSHR_ATTACHED_DATA_TYPE_JITPROFILE == ADWTYPE(adw))
							|| (OMRSHR_ATTACHED_DATA_TYPE_JITHINT == ADWTYPE(adw))) {
						MethodNameAndSignature* methodNameAndSignature = (MethodNameAndSignature*)ADWCACHEOFFSET(((AttachedDataWrapper*)ITEMDATA(it)));
//						J9ClassLoader* loader;
//						OMRUTF8* romClassName = NULL;
//						J9ROMClass* romClass = vmFunctions->findROMClassFromPC(currentThread, (UDATA)methodNameAndSignature, &loader);
//						OMRUTF8* methodNameAndSignatureName = J9ROMMETHOD_GET_NAME(romClass, methodNameAndSignature);
...
//						if (methodNameAndSignatureName) {
//
//							if ((0 != (showFlags & PRINTSTATS_SHOW_JITHINT)) && (OMRSHR_ATTACHED_DATA_TYPE_JITHINT == ADWTYPE(adw))) {
//								CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
//										J9NLS_SHRC_CM_PRINTSTATS_ATTACHED_JITHINT_DISPLAY, ITEMJVMID(it), 
//										(UDATA)it, OMRUTF8_LENGTH(methodNameAndSignatureName), OMRUTF8_DATA(methodNameAndSignatureName));
//							} else if ((0 != (showFlags & PRINTSTATS_SHOW_JITPROFILE)) && (OMRSHR_ATTACHED_DATA_TYPE_JITPROFILE == ADWTYPE(adw))){
//								CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
...
//						if (methodNameAndSignatureSig) {
//							CACHEMAP_PRINT3((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
//									J9NLS_SHRC_CM_PRINTSTATS_JITPROFILE_DISPLAY_SIG_ADDR, OMRUTF8_LENGTH(methodNameAndSignatureSig), OMRUTF8_DATA(methodNameAndSignatureSig), methodNameAndSignature);
//						}
//
//						if (isStale) {
//							omrtty_printf(" ");
...
//						if (romClassName) {
//							CACHEMAP_PRINT3(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
//									J9NLS_SHRC_CM_PRINTSTATS_JITPROFILE_FROM_ROMCLASS, OMRUTF8_LENGTH(romClassName), OMRUTF8_DATA(romClassName), romClass);
//						}
					}
				}
				break;
 ...
 */				   
IDATA
SH_CacheMap::printCacheStats(OMR_VMThread* currentThread, UDATA showFlags, U_64 runtimeFlags)
{	
...
//	if ( 1 == getJavacoreData(currentThread->_vm, &javacoreData) ) {
...
//		
//		omrtty_printf("\t");
//		if (OMR_ARE_ALL_BITS_SET(javacoreData.feature, OMRSH_FEATURE_COMPRESSED_POINTERS)) {
//			CACHEMAP_PRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_FEATURE, "cr");
//		} else if (OMR_ARE_ALL_BITS_SET(javacoreData.feature, OMRSH_FEATURE_NON_COMPRESSED_POINTERS)) {
...
//		if (runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
...
{
	return ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_BYTECODE_AGENT_RUNNING) != 0);
}
...
SH_CacheMap::printShutdownStats(void)
{
	...
	CACHEMAP_TRACE2(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_PRINTSHUTDOWNSTATS_READ_STORED, bytesRead, bytesStored);
	CACHEMAP_TRACE3(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_PRINTSHUTDOWNSTATS_UNSTORED_V1, softmxUnstoredBytes, maxAOTUnstoredBytes, maxJITUnstoredBytes);
}
 ...
 */
void
SH_CacheMap::runExitCode(OMR_VMThread* currentThread)
{
	SH_Manager* walkManager;
	SH_Managers::ManagerWalkState state;
	SH_CompositeCacheImpl* cache = _ccHead;
...
//	J9PortLibrary* _portlib = currentThread->_vm->portLibrary;
//
//	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
//
...
//		*pathBuf = (char*)omrmem_allocate_memory(sizeNeeded, OMRMEM_CATEGORY_CLASSES);
//		fullPath = *pathBuf;
//		if (!fullPath) {
//			if (config->verboseFlags & OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT) {
...
//
IDATA 
SH_CacheMap::enterStringTableMutex(OMR_VMThread* currentThread, BOOLEAN readOnly, UDATA* doRebuildLocalData, UDATA* doRebuildCacheData)
{
	IDATA result = -1;
 ...
 */
const OMRUTF8*
SH_CacheMap::getCachedUTFString(OMR_VMThread* currentThread, const char* local, U_16 localLen)
{
	const char* fnName = "getCachedUTFString";
	const OMRUTF8* pathUTF = NULL;
	U_8 temp[OMRSH_MAXPATH + sizeof(OMRUTF8)];
	OMRUTF8* temputf = (OMRUTF8*)&temp;
	char* tempstr = (char*)&(OMRUTF8_DATA(temputf));
	SH_ScopeManager* localSCM;
	...
	OMRUTF8_SET_LENGTH(temputf, localLen);
	strncpy(tempstr, local, OMRUTF8_LENGTH(temputf));
	...
	if (NULL == pathUTF) {
		if (allowUpdate){
			...
			increaseUnstoredBytes(OMRUTF8_LENGTH(temputf) + sizeof(struct OMRUTF8));
		}
	}
 ...
 */
//void 
//SH_CacheMap::notifyClasspathEntryStateChange(OMR_VMThread* currentThread, const char* path, UDATA newState)
//{
//	const OMRUTF8* pathUTF;
//	SH_ClasspathManager* localCPM;
//
//	if (!(localCPM = getClasspathManager(currentThread))) { 
//		return;
...
SH_ScopeManager*
SH_CacheMap::getScopeManager(OMR_VMThread* currentThread)
{
...
//SH_ClasspathManager*
//SH_CacheMap::getClasspathManager(OMR_VMThread* currentThread)
//{
...
//
//SH_ROMClassManager*
//SH_CacheMap::getROMClassManager(OMR_VMThread* currentThread)
//{
...
SH_ByteDataManager*
SH_CacheMap::getByteDataManager(OMR_VMThread* currentThread)
{
...
SH_CacheMap::getCompiledMethodManager(OMR_VMThread* currentThread)
{
...
SH_CacheMap::getAttachedDataManager(OMR_VMThread* currentThread)
{
...
UDATA
SH_CacheMap::startAllManagers(OMR_VMThread* currentThread)
{
...
SH_CompositeCacheImpl*
SH_CacheMap::initCachelet(OMR_VMThread* currentThread, BlockPtr existingCacheletMemory, bool creatingCachelet) 
{
	...
	newCachelet = SH_CompositeCacheImpl::newInstanceNested(currentThread->_vm, _cc, (SH_CompositeCacheImpl*)ccMem, OMRSHR_DEFAULT_CACHELET_SIZE, existingCacheletMemory, creatingCachelet);
	if (!newCachelet) {
		pool_removeElement(_ccPool, ccMem);
		return NULL;
	}
...
//IDATA
//SH_CacheMap::startupCachelet(OMR_VMThread* currentThread, SH_CompositeCache* cachelet)
//{
//	IDATA rc;
//	
...
//		if (rc == CC_STARTUP_OK) {
//			bool hasClassSegmentMutex = 
//				(omrthread_monitor_owned_by_self(currentThread->_vm->classMemorySegments->segmentMutex) != 0);
...
//	descriptor.type = OMRSHR_DATA_TYPE_CACHELET;
//	descriptor.length = OMRSHR_DEFAULT_CACHELET_SIZE;
//	descriptor.flags = (OMRSHRDATA_ALLOCATE_ZEROD_MEMORY | OMRSHRDATA_NOT_INDEXED);
//
//	for (i=0; i<2; i++) {
...
//
//							j9shr_resetSharedStringTable(currentThread->_vm);
//
//							_cc->exitReadWriteAreaMutex(currentThread, OMRSHR_STRING_POOL_OK);
//							resetAllManagers(currentThread);
//						}
//					}
//					break;
...
/* THREADING: Must have cache write mutex */
SH_CompositeCacheImpl*
SH_CacheMap::createNewChainedCache(OMR_VMThread* currentThread, UDATA requiredSize)
{
	void* ccMem;
	...
	ccMem = omrmem_allocate_memory(SH_CompositeCacheImpl::getRequiredConstrBytes(false, false), OMRMEM_CATEGORY_CLASSES);
	...
	newCache = SH_CompositeCacheImpl::newInstanceChained(currentThread->_vm, (SH_CompositeCacheImpl*)ccMem, _sharedClassConfig, J9PORT_SHR_CACHE_TYPE_VMEM);
	if (newCache) {
		OMR_VM* vm = currentThread->_vm;
		U_32 ignored;
		IDATA rc;
		...
		if (rc == CC_STARTUP_OK) {
			...
			_cc->setCacheHeaderFullFlags(currentThread, OMRSHR_ALL_CACHE_FULL_BITS, false);
			/* Mark existing cachelets full */
			while (walk) {
				if (false == walk->isCacheMarkedFull(currentThread)) {
					walk->setCacheHeaderFullFlags(currentThread, OMRSHR_ALL_CACHE_FULL_BITS, false);
				}
				last = walk;
				walk = walk->getNext();
			}
 ...
 */
IDATA
SH_CacheMap::buildCacheletMetadata(OMR_VMThread* currentThread, SH_Manager::CacheletMetadataArray** metadataArray)
{
	 ...
	 */
	array = (SH_Manager::CacheletMetadataArray*)omrmem_allocate_memory(metadataArraySize, OMRMEM_CATEGORY_CLASSES);
	if (array == NULL) {
		_ccHead->exitWriteMutex(currentThread, fnName);
		return -1;
	}
...
void
SH_CacheMap::freeCacheletMetadata(OMR_VMThread* currentThread, SH_Manager::CacheletMetadataArray* metaArray)
{
	OMRPORT_ACCESS_FROM_VMC(currentThread->_vm);
	SH_Manager* walkManager;
	SH_Managers::ManagerWalkState state;
	...
	omrmem_free_memory(metaArray);
}

IDATA
SH_CacheMap::readCacheletHints(OMR_VMThread* currentThread, SH_CompositeCacheImpl* cachelet, CacheletWrapper* cacheletWrapper)
{
	UDATA counter = 0;
 ...
 */
bool
SH_CacheMap::readCacheletSegments(OMR_VMThread* currentThread, SH_CompositeCacheImpl* cachelet, CacheletWrapper* cacheletWrapper)
{
	...
	Trc_SHR_Assert_True(omrthread_monitor_owned_by_self(currentThread->_vm->classMemorySegments->segmentMutex));
...
IDATA
SH_CacheMap::writeCacheletMetadata(OMR_VMThread* currentThread, SH_Manager::CacheletMetadata* cacheletMetadata)
{
	...
	if (itemInCache == NULL) {
		...
		if (true == _cc->isCacheMarkedFull(currentThread)) {
			...
			j9tty_printf(OMRPORTLIB, "Free bytes: %d\t", _cc->getFreeBytes());
			j9tty_printf(OMRPORTLIB, "AOT bytes: %d\t", _cc->getAOTBytes());
			j9tty_printf(OMRPORTLIB, "JIT bytes: %d\n", _cc->getJITBytes());
			if (1 == getJavacoreData(currentThread->_vm, &descriptor)) {
				j9tty_printf(OMRPORTLIB, "minAOT: %d\t", descriptor.minAOT);
				j9tty_printf(OMRPORTLIB, "minJIT: %d\n", descriptor.minJIT);
			}
		}
 ...
 */
IDATA
SH_CacheMap::growCacheInPlace(OMR_VMThread* currentThread, UDATA rwGrowth, UDATA freeGrowth)
{
 ...
 */
bool
SH_CacheMap::serializeSharedCache(OMR_VMThread* currentThread)
{
	bool ok = true;
 ...
 */
bool
SH_CacheMap::serializeOfflineCache(OMR_VMThread* currentThread)
{
	...
	objectMemory = omrmem_allocate_memory(ccMemorySize, OMRMEM_CATEGORY_CLASSES);
	...
	*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT;			/* TODO: HACK FOR NOW */
...
serializeOfflineCache_done:
	...
	omrmem_free_memory(objectMemory);
	return ok;
}
 ...
 */
IDATA
SH_CacheMap::fixupCompiledMethodsForSerialization(OMR_VMThread* currentThread, void* serializedROMClassStartAddress)
{
	IDATA rc = 0;
	SH_CompositeCacheImpl *cachelet;
...
void
SH_CacheMap::fixupCacheletMetadata(OMR_VMThread* currentThread,
...
#if defined(OMRSHR_CACHELETS_SAVE_READWRITE_AREA)
void
SH_CacheMap::fixCacheletReadWriteOffsets(OMR_VMThread* currentThread)
{
	ShcItem* it = NULL;
...
SH_CacheMap::resetCacheDescriptorList(OMR_VMThread* currentThread, OMRSharedClassConfig* sharedClassConfig)
{
	...
	while (nextCacheDesc != cacheDesc) {
		...
		omrmem_free_memory(nextCacheDesc);
 ...
 */
IDATA
SH_CacheMap::startClassTransaction(OMR_VMThread* currentThread, bool lockCache, const char* caller) {
	IDATA retval;
 ...
 */
IDATA
SH_CacheMap::exitClassTransaction(OMR_VMThread* currentThread, const char* caller) {
 ...
 */
IDATA
SH_CacheMap::startupForStats(OMR_VMThread* currentThread, SH_OSCache * oscache, U_64 * runtimeflags)
{
	IDATA retval = 0;
 ...
 */
IDATA
SH_CacheMap::shutdownForStats(OMR_VMThread* currentThread)
{
	SH_Manager* walkManager;
	SH_Managers::ManagerWalkState state;
	IDATA retval = 0;
 ...
 */
IDATA
SH_CacheMap::startupCacheletForStats(OMR_VMThread* currentThread, SH_CompositeCache* cachelet)
{
	IDATA rc;
 ...
 */
static char*
formatAttachedDataString(OMR_VMThread* currentThread, U_8 *attachedData, UDATA attachedDataLength,
	...
	*stringCursor = '\0'; /* handle the zero-length case */
	while ((bytesRemaining > 0) && ((stringCursor+BYTE_STRING_LENGTH) < stringBufferEnd)){
		stringCursor += omrstr_printf(stringCursor, bufferLength, "0x%#02x ", *dataCursor); /* increment does not include the trailing '\0' */
		++dataCursor;
		--bytesRemaining;
	}
 ...
 * @param[in] currentThread The current OMR_VMThread
 ...
 */
IDATA
SH_CacheMap::aotMethodOperation(OMR_VMThread* currentThread, char* methodSpecs, UDATA action)
{
	IDATA numMethods = 0;
	...
	if (numSpecs <= 0) {
		...
		CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_PARSE_METHOD_SPECS_ERROR);
		return -1;
		...
		CACHEMAP_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_PARSE_METHOD_SPECS_TOO_MANY_ERROR, SHR_METHOD_SPEC_TABLE_MAX_SIZE);
		return -1;
	}
 ...
 */
const IDATA
SH_CacheMap::aotMethodOperationHelper(OMR_VMThread* currentThread, MethodSpecTable* specTable, IDATA numSpecs, UDATA action)
{
	IDATA rc = 0;
	ShcItem* it = NULL;
//	J9InternalVMFunctions* vmFunctions = currentThread->_vm->internalVMFunctions;
	...
	if (0 != _ccHead->enterWriteMutex(currentThread, lockCache, fnName)) {
		...
		CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_ENTER_WRITE_MUTEX);
		return -1;
	}
	...
	do {
		...
		if (TYPE_COMPILED_METHOD == ITEMTYPE(it) || TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)) {
			MethodNameAndSignature* methodNameAndSignature = (MethodNameAndSignature*)CMWROMMETHOD(((CompiledMethodWrapper*)ITEMDATA(it)));
//			OMRUTF8* romClassName = NULL;
//			J9ClassLoader* loader = NULL;
//			J9ROMClass* romClass = vmFunctions->findROMClassFromPC(currentThread, (UDATA)methodNameAndSignature, &loader);
	 		OMRUTF8* methodNameAndSignatureName = OMRNAMEANDSIG_NAME(methodNameAndSignature); //J9ROMMETHOD_GET_NAME(romClass, methodNameAndSignature);
			OMRUTF8* methodNameAndSignatureSig = OMRNAMEANDSIG_SIGNATURE(methodNameAndSignature); // J9ROMMETHOD_GET_SIGNATURE(romClass, methodNameAndSignature);

//			if (NULL != romClass) {
//				romClassName = J9ROMCLASS_CLASSNAME(romClass);
//			}

			if (matchAotMethod(specTable, numSpecs, methodNameAndSignatureName, methodNameAndSignatureSig)) {
		 		if (NULL != methodNameAndSignatureName) {
		 			CACHEMAP_TRACE4(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
							J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY, ITEMJVMID(it), (UDATA)it, OMRUTF8_LENGTH(methodNameAndSignatureName), OMRUTF8_DATA(methodNameAndSignatureName));
		 		}

				if (NULL != methodNameAndSignatureSig) {
					CACHEMAP_TRACE3(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
							J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY_SIG_ADDR, OMRUTF8_LENGTH(methodNameAndSignatureSig), OMRUTF8_DATA(methodNameAndSignatureSig), methodNameAndSignature);
				}

				if (_ccHead->stale((BlockPtr)ITEMEND(it))) {
					omrtty_printf(" ");
					CACHEMAP_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
				}

				if ((SHR_FIND_AOT_METHOTHODS == action) && TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)
				) {
...
//					CACHEMAP_TRACE3(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
//							J9NLS_SHRC_CM_PRINTSTATS_AOT_FROM_ROMCLASS, OMRUTF8_LENGTH(romClassName), OMRUTF8_DATA(romClassName), romClass);
//				}
 ...
 * @param[in] methodNameAndSignatureName The name of the method
 * @param[in] methodNameAndSignatureSig The signature of the method
 *
 * @return true if the method matches any specification, false otherwise
 */
const bool
SH_CacheMap::matchAotMethod(MethodSpecTable* specTable, IDATA numSpecs, OMRUTF8* methodNameAndSignatureName, OMRUTF8* methodNameAndSignatureSig)
{
	bool rc = false;
...
//							(const char *)OMRUTF8_DATA(romClassName), OMRUTF8_LENGTH(romClassName)));
		}
//		if (false == classMatch) {
//			continue;
//		}
		...
		if (NULL == specTable[i].methodName) {
			/* Any methods */
			methodMatch = true;
		} else if (NULL == methodNameAndSignatureName) {
			...
			methodMatch = (TRUE == wildcardMatch(specTable[i].methodNameMatchFlag,
										...
										(const char *)OMRUTF8_DATA(methodNameAndSignatureName), OMRUTF8_LENGTH(methodNameAndSignatureName)));
		}
		...
		if (NULL == specTable[i].methodSig) {
			/* Any signature */
			sigMatch = true;
		} else if (NULL == OMRUTF8_DATA(methodNameAndSignatureSig)) {
			/* Signature is missing */
			sigMatch = false;
		} else {
			char* methodSig = (char *)OMRUTF8_DATA(methodNameAndSignatureSig);
 ...
 *
 * @param [in] currentThread pointer to current OMR_VMThread
 *
 * @return void
 */
void
SH_CacheMap::protectPartiallyFilledPages(OMR_VMThread *currentThread)
{
 ...
 *
 * @return I_32	OMRSHR_SOFTMX_ADJUSTED is set if softmx has been adjusted
 *				OMRSHR_MIN_AOT_ADJUSTED is set if minAOT has been adjusted
 *				OMRSHR_MAX_AOT_ADJUSTED is set if maxAOT has been adjusted
 *				OMRSHR_MIN_JIT_ADJUSTED is set if minJIT has been adjusted
 *				OMRSHR_MAX_JIT_ADJUSTED is set if maxJIT has been adjusted
 */
I_32
SH_CacheMap::tryAdjustMinMaxSizes(OMR_VMThread* currentThread, bool isJCLCall)
{
 ...
 */
void
SH_CacheMap::updateRuntimeFullFlags(OMR_VMThread* currentThread)
{
	_ccHead->updateRuntimeFullFlags(currentThread);
	return;
}
...
SH_CacheMap::increaseUnstoredBytes(U_32 blockBytes, U_32 aotBytes, U_32 jitBytes)
{
	...
	if (OMR_ARE_ANY_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES)) {
		return;
	}
