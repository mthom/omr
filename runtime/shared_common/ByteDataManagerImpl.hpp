...
public:
	...
	static SH_ByteDataManagerImpl* newInstance(OMR_VM* vm, SH_SharedCache* cache, SH_ByteDataManagerImpl* memForConstructor);
	...
	virtual ByteDataWrapper* findSingleEntry(OMR_VMThread* currentThread, const char* key, UDATA keylen, UDATA dataType, U_16 jvmID, UDATA* dataLen);
...
protected:
	...
	virtual IDATA localInitializePools(OMR_VMThread* currentThread);

	virtual void localTearDownPools(OMR_VMThread* currentThread);
...
private:
	...
	UDATA _indexedBytesByType[OMRSHR_DATA_TYPE_MAX+1];
	UDATA _numIndexedBytesByType[OMRSHR_DATA_TYPE_MAX+1];
	...
	{
	protected:
		void localInit(const OMRUTF8* key, const ShcItem* item) {};
	};
};
