...
#if !defined(OS_CACHE_REGION_RETREATING_BUMP_FOCUS_HPP_INCLUDED)
#define OS_CACHE_REGION_RETREATING_BUMP_FOCUS_HPP_INCLUDED
...
template <typename T>
class OSCacheRegionRetreatingBumpFocus: public OSCacheRegionFocus<T> {
public:
  OSCacheRegionRetreatingBumpFocus(OSCacheRegion* region)
    : OSCacheRegionFocus<T>(region, (T*)((U_8*)region->regionStartAddress() + region->regionSize() - sizeof(T)))
  {}
  
  OSCacheRegionRetreatingBumpFocus(OSCacheRegion* region, T* focus)
  ...
  OSCacheRegionRetreatingBumpFocus<T>& operator -=(UDATA bump) {
  ...
  inline bool operator >(const OSCacheRegionRetreatingBumpFocus<T>& rhs) const {
