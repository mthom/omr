...
#define M_ERR_TRACE(var) if (_verboseFlags) omrnls_printf(J9NLS_ERROR, var)
#define M_ERR_TRACE2(var, p1, p2) if (_verboseFlags) omrnls_printf(J9NLS_ERROR, var, p1, p2)
#define M_ERR_TRACE_NOTAG(var) if (_verboseFlags) omrnls_printf((J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var)
#define M_ERR_TRACE2_NOTAG(var, p1, p2) if (_verboseFlags) omrnls_printf((J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var, p1, p2)
#define M_ERR_TRACE3_NOTAG(var, p1, p2, p3) if (_verboseFlags) omrnls_printf((J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var, p1, p2, p3)
...
public:
	...
	public:
		...
		void initialize(const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet, UDATA hashPrimeValue);

	protected:
		/* Override localInit to perform subclass initialization */
		void localInit(const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet, UDATA hashPrimeValue) {};
	};
	...
	virtual bool storeNew(OMR_VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet) = 0;
	
	void getNumItems(OMR_VMThread* currentThread, UDATA* nonStaleItems, UDATA* staleItems);
	
	IDATA reset(OMR_VMThread* currentThread);
	
	void cleanup(OMR_VMThread* currentThread);

	void shutDown(OMR_VMThread* currentThread);

	IDATA startup(OMR_VMThread* currentThread, U_64* runtimeFlags, UDATA verboseFlags, UDATA cacheSize);

	U_8 getState();
	
#if defined(J9SHR_CACHELET_SUPPORT)
	SH_CompositeCacheImpl* getCacheAreaForData(SH_CacheMap* cm, OMR_VMThread* currentThread, UDATA dataType, UDATA dataLength);
...
#if defined(J9SHR_CACHELET_SUPPORT)
	IDATA primeFromHints(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, U_8* hintsData, UDATA datalength);

	IDATA generateHints(OMR_VMThread* vmthread, CacheletMetadataArray* metadataArray);
	
	virtual void fixupHintsForSerialization(OMR_VMThread* vmthread,
			UDATA dataType, UDATA dataLen, U_8* data,
			IDATA deployedOffset, void* serializedROMClassStartAddress) {}
#endif
  
#if defined(J9SHR_CACHELET_SUPPORT)
	IDATA freeHintData(OMR_VMThread* vmthread, CacheletMetadataArray* metadataArray);
#endif
...
protected:
	...
	HashLinkedListImpl* createLink(const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet, UDATA hashPrimeValue, const J9Pool* allocationPool);

	/* This function is the lookup entry point for a manager linked list hashtable */
	HashLinkedListImpl* hllTableLookup(OMR_VMThread* currentThread, const char* name, U_16 nameLen, bool allowCacheletStartup);

	/* This function is the update entry point for a manager linked list hashtable */
	HashLinkedListImpl* hllTableUpdate(OMR_VMThread* currentThread, const J9Pool* linkPool, const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet);

	/* Synchronize access to _hashTable */
	bool lockHashTable(OMR_VMThread* currentThread, const char* funcName);
	void unlockHashTable(OMR_VMThread* currentThread, const char* funcName);
	...
	IDATA hllCollectHashes(OMR_VMThread* currentThread, SH_CompositeCache* cachelet, CacheletHints* hints);
#endif
#if defined(J9SHR_CACHELET_SUPPORT)
	virtual IDATA primeHashtables(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, U_8* hintsData, UDATA datalength);
	virtual IDATA createHintsForCachelet(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, CacheletHints* hints);
	IDATA startupHintCachelets(OMR_VMThread* currentThread, UDATA hint);
#endif
...
private:
	...
	IDATA initializeHashTable(OMR_VMThread* currentThread);

	void tearDownHashTable(OMR_VMThread* currentThread);

	HashLinkedListImpl* hllTableAdd(OMR_VMThread* currentThread, const J9Pool* linkPool, const OMRUTF8* key, const ShcItem* item,
					UDATA hashPrimeValue, SH_CompositeCache* cachelet, HashLinkedListImpl** addToList);
	HashLinkedListImpl* hllTableLookupHelper(OMR_VMThread* currentThread, U_8* key, U_16 keySize, UDATA hashValue, SH_CompositeCache* cachelet);
  ...
  // OMR does not have an 'internal function table' as far as I can see.
