...
public:
	...
	void *operator new(size_t size, void *memoryPtr) { return memoryPtr; }
        ...
        static SH_OSCache* newInstance(OMRPortLibrary* portlib, SH_OSCache* memForConstructor, const char* cacheName, UDATA generation, J9PortShcVersion* versionData);

	static UDATA getRequiredConstrBytes(void);
	
	static IDATA getCacheDir(OMR_VM* vm, const char* ctrlDirName, char* buffer, UDATA bufferSize, U_32 cacheType);
	
	static IDATA createCacheDir(OMRPortLibrary* portLibrary, char* cacheDirName, UDATA cacheDirPerm, bool cleanMemorySegments);

	static void getCacheVersionAndGen(OMRPortLibrary* portlib, OMR_VM* vm, char* buffer, UDATA bufferSize, const char* cacheName,
			J9PortShcVersion* versionData, UDATA generation, bool isMemoryType);

	static UDATA getCurrentCacheGen(void);

	static UDATA statCache(OMRPortLibrary* portLibrary, const char* cacheDirName, const char* cacheNameWithVGen, bool displayNotFoundMsg);

	static J9Pool* getAllCacheStatistics(OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA localVerboseFlags, UDATA j2seVersion, bool includeOldGenerations, bool ignoreCompatible, UDATA reason, bool isCache);
	static IDATA getCacheStatistics(OMR_VM* vm, const char* ctrlDirName, const char* cacheNameWithVGen, UDATA groupPerm, UDATA localVerboseFlags, UDATA j2seVersion, SH_OSCache_Info* result, UDATA reason);
	...
	virtual void *attach(OMR_VMThread* currentThread, J9PortShcVersion* expectedVersionData) = 0;
	...
	virtual IDATA setRegionPermissions(struct OMRPortLibrary* portLibrary, void *address, UDATA length, UDATA flags) = 0;
	
	virtual UDATA getPermissionsRegionGranularity(struct OMRPortLibrary* portLibrary) = 0;

	virtual U_32 getDataSize();

	virtual U_32 getTotalSize() = 0;

  //	virtual UDATA getJavacoreData(OMR_VM *vm, J9SharedClassJavacoreDataDescriptor* descriptor) = 0;
...
protected:	
	...
	virtual void initialize(OMRPortLibrary* portLib_, char* memForConstructor, UDATA generation) = 0;
	...
	static bool getCacheStatsCommon(OMR_VM* vm, SH_OSCache *cache, SH_OSCache_Info *cacheInfo);
...
private:
	void setEnableVerbose(OMRPortLibrary* portLib, OMR_VM* vm, J9PortShcVersion* versionData, char* cacheNameWithVGen);
