...
SH_AttachedDataManagerImpl::newInstance(OMR_VM* vm, SH_SharedCache* cache_, SH_AttachedDataManagerImpl* memForConstructor)
{
...
void
SH_AttachedDataManagerImpl::initialize(OMR_VM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
	Trc_SHR_ADMI_initialize_Entry();

	_cache = cache_;
	_portlib = vm->_runtime->_portLibrary; // vm->portLibrary;
	_htMutex = NULL;
	...
	_rrmHashTableName = OMR_GET_CALLSITE();
...
SH_AttachedDataManagerImpl::getNumOfType(UDATA type) {
	if (type > OMRSHR_ATTACHED_DATA_TYPE_MAX) {
 ...
 */
bool
SH_AttachedDataManagerImpl::storeNew(OMR_VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
	bool rc;
	UDATA type;
	...
	if (type <= OMRSHR_ATTACHED_DATA_TYPE_MAX) {
		...
		/* Unknown type */
		_bytesByType[OMRSHR_ATTACHED_DATA_TYPE_UNKNOWN] += ITEMDATALEN(itemInCache);
		_numBytesByType[OMRSHR_ATTACHED_DATA_TYPE_UNKNOWN] += 1;
	}
