...
public:
	...
	virtual bool storeNew(OMR_VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet);

	virtual const void* findResource(OMR_VMThread* currentThread, UDATA resourceKey);
	 ...
	 */
	inline UDATA existsResourceForROMAddress(OMR_VMThread* currentThread, UDATA resourceKey) {
	...
	virtual UDATA markStale(OMR_VMThread* currentThread, UDATA resourceKey, const ShcItem* itemInCache);
	
	virtual bool permitAccessToResource(OMR_VMThread* currentThread);
...
class SH_ResourceDescriptor
{
	public:
		...
		virtual U_16 getResourceDataSubType() { return OMRSHR_RESOURCE_TYPE_UNKNOWN; }
...
protected:
	...
	*/
	...
	IDATA localPostStartup(OMR_VMThread* currentThread) { return 0; };

	void localPostCleanup(OMR_VMThread* currentThread) {};

	virtual J9HashTable* localHashTableCreate(OMR_VMThread* currentThread, U_32 initialEntries);

	IDATA localInitializePools(OMR_VMThread* currentThread) { return 0; };

	void localTearDownPools(OMR_VMThread* currentThread) {};
...
private:
	...
	HashTableEntry* rrmTableAdd(OMR_VMThread* currentThread, const ShcItem* item, SH_CompositeCache* cachelet);
#if defined(OMRSHR_CACHELET_SUPPORT)
	HashTableEntry* rrmTableAddPrime(OMR_VMThread* currentThread, UDATA key, SH_CompositeCache* cachelet);
#endif
	HashTableEntry* rrmTableAddHelper(OMR_VMThread* currentThread, HashTableEntry* newEntry, SH_CompositeCache* cachelet);
	HashTableEntry* rrmTableLookup(OMR_VMThread* currentThread, UDATA key);
	UDATA rrmTableRemove(OMR_VMThread* currentThread, UDATA key);
	...
	IDATA rrmCollectHashes(OMR_VMThread* currentThread, SH_CompositeCache* cachelet, CacheletHints* hints);
#endif
