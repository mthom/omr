...
#define CC_TRACE(verboseLevel, nlsFlags, var1) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1)
#define CC_TRACE1(verboseLevel, nlsFlags, var1, p1) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1)
#define CC_TRACE2(verboseLevel, nlsFlags, var1, p1, p2) if (_verboseFlags & verboseLevel) omrnls_printf(nlsFlags, var1, p1, p2)
#define CC_ERR_TRACE(var) if (_verboseFlags) omrnls_printf(J9NLS_ERROR, var)
#define CC_ERR_TRACE1(var, p1) if (_verboseFlags) omrnls_printf(J9NLS_ERROR, var, p1)
#define CC_ERR_TRACE2(var, p1, p2) if (_verboseFlags) omrnls_printf(J9NLS_ERROR, var, p1, p2)
#define CC_WARNING_TRACE(var, isJCLCall) if (_verboseFlags && !isJCLCall) omrnls_printf(J9NLS_WARNING, var)
#define CC_WARNING_TRACE1(var, p1, isJCLCall) if (_verboseFlags && !isJCLCall) omrnls_printf(J9NLS_WARNING, var, p1)
#define CC_INFO_TRACE(var, isJCLCall) if (_verboseFlags && !isJCLCall) omrnls_printf(J9NLS_INFO, var)
#define CC_INFO_TRACE1(var, p1, isJCLCall) if (_verboseFlags && !isJCLCall) omrnls_printf(J9NLS_INFO, var, p1)
#define CC_INFO_TRACE3(var, p1, p2, p3) if (_verboseFlags) omrnls_printf(J9NLS_INFO, var, p1, p2, p3)
 ...
 */
SH_CompositeCacheImpl*
SH_CompositeCacheImpl::newInstanceNested(OMR_VM* vm, SH_CompositeCacheImpl* parent, SH_CompositeCacheImpl* memForConstructor, UDATA nestedSize, BlockPtr nestedMemory, bool creatingCachelet)
{
	SH_CompositeCacheImpl* newCC = (SH_CompositeCacheImpl*)memForConstructor;
...
void
SH_CompositeCacheImpl::commonInit(OMR_VM* vm)
{
	_started = false;
	_cacheName = NULL;
...
void
SH_CompositeCacheImpl::initializeNested(OMR_VM* vm, SH_CompositeCacheImpl* parent, BlockPtr memForConstructor, UDATA nestedSize, BlockPtr nestedMemory, bool creatingCachelet)
{
...
//	setCurrentCacheVersion(vm, J2SE_VERSION(vm), &versionData);
	...
	if ((UnitTest::NO_TEST != UnitTest::unitTest) && (UnitTest::CORRUPT_CACHE_TEST != UnitTest::unitTest)) {
		 ...
		 */
		_osPageSize = (omrmmap_capabilities() & OMRPORT_MMAP_CAPABILITY_PROTECT) ? omrmmap_get_region_granularity(UnitTest::cacheMemory) : 0;
		_newHdrPtr = (SH_SharedCacheHeaderInit*)((BlockPtr)memForConstructor);
...
//		_debugData->initialize();
		...
		if ((sharedClassConfig->runtimeFlags & OMRSHR_RUNTIMEFLAG_CREATE_OLD_GEN) != 0) {
 ...
 */
void
SH_CompositeCacheImpl::cleanup(OMR_VMThread* currentThread)
{
 ...
 */
void
SH_CompositeCacheImpl::reset(OMR_VMThread* currentThread)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
 ...
 *
 * @see setCorruptCache(OMR_VMThread *, IDATA, UDATA)
 */
void
SH_CompositeCacheImpl::setCorruptCache(OMR_VMThread* currentThread)
{
	...
	if ( !((UnitTest::CORRUPT_CACHE_TEST == UnitTest::unitTest) && (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_STATS))) ) {
		/* This function may be called during shrtest with OMRSHR_RUNTIMEFLAG_ENABLE_STATS. In this case
 ...
 *
 * @see setCorruptCache(OMR_VMThread* currentThread)
 *
 ...
 */
void
SH_CompositeCacheImpl::setCorruptCache(OMR_VMThread *currentThread, IDATA corruptionCode, UDATA corruptValue)
{
	...
	if (1 == ccToUse->_theca->corruptFlag) {
		...
		if (0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_FORCE_DUMP_IF_CORRUPT)) {
	 ...
	 */
//	if ((TRUE == runCorruptCacheHook) && (0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_DISABLE_CORRUPT_CACHE_DUMPS))) {
...
SH_CompositeCacheImpl::setCacheAreaBoundaries(OMR_VMThread* currentThread, OMRSharedCachePreinitConfig* piConfig)
{
	...
	if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) {
		...
		omrtty_printf("Page size rounding not supported\n");
	}
...
//		ClassDebugDataProvider::HeaderInit(_theca, 0);
	...
	if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) && (isVerbosePages() == true)) {
		omrtty_printf("New cache rounded to page size of %d bytes\n", _osPageSize);
		omrtty_printf("   CompositeCache header starts at %p\n", _theca);
		omrtty_printf("   ReadWrite area starts at %p and is %d bytes\n", READWRITEAREASTART(_theca), READWRITEAREASIZE(_theca));
		omrtty_printf("   ROMClass segment starts at %p\n", CASTART(_theca));
		omrtty_printf("   Debug Region starts at %p and is %d bytes\n", CADEBUGSTART(_theca), _theca->debugRegionSize);
		omrtty_printf("   Cache ends at %p\n", CAEND(_theca));
		omrtty_printf("   Cache soft max bytes is %d\n", (IDATA)_theca->softMaxBytes);
	}
...
SH_CompositeCacheImpl::notifyPagesRead(BlockPtr start, BlockPtr end, UDATA expectedDirection, bool protect)
{
	...
	if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT) && !isLocked()) {
		...
		if (protectStart != protectEnd) {
			IDATA rc = setRegionPermissions(_portlib, (void*)protectStart, (protectEnd - protectStart), (doProtect ? OMRPORT_PAGE_PROTECT_READ : (OMRPORT_PAGE_PROTECT_WRITE | OMRPORT_PAGE_PROTECT_READ)));
			OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
			...
			if (isVerbosePages() == true) {
				omrtty_printf("Set memory region permissions in notifyPagesRead for %p to %p - doProtect=%d - rc=%d\n", protectStart, protectEnd, doProtect, rc);
			}
		}
...
SH_CompositeCacheImpl::notifyPagesCommitted(BlockPtr start, BlockPtr end, UDATA expectedDirection)
{
	...
	if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MSYNC) {
		UDATA actualDirection = (start < end) ? DIRECTION_FORWARD : DIRECTION_BACKWARD;
		...
		UDATA* localCrashCntr, bool isFirstStart, bool *cacheHasIntegrity)
{
	...
	*cacheHasIntegrity = false;
	...
	if (_parent == NULL) {
		if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_GROUP_ACCESS) {
		...
		} else if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NONFATAL) {
			openMode |= J9OSCACHE_OPEN_MODE_TRY_READONLY_ON_FAIL;
		}
		if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_AUTOKILL_DIFF_BUILDID) {
		...
		if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_DETECT_NETWORK_CACHE) {
	...
	if (isFirstStart) {
		...
		if (_osPageSize == 0) {
			...
			*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE;
		}
		if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) {
	...
	*actualSize = 0;
	...
	if (cacheMemory != NULL) {
		...
		if (_parent == NULL) {
			...
			/* Set the readOnly state for testing that sets the OMRSHR_RUNTIMEFLAG_ENABLE_READONLY flag */
...
//		setCurrentCacheVersion(vm, J2SE_VERSION(currentThread->_vm), &versionData);
		...
		if ((OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID))
			 ...
			 * OMRSHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID will be used in j9shr_createCacheSnapshot()
			 */
			(*_runtimeFlags) &= ~(OMRSHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID);
		}
	...
	if (hasWriteMutex) {
		...
		if (cacheMemory != NULL) {
			...
			if ((_theca != 0) && getContainsCachelets() &&
					((OMRSHR_RUNTIMEFLAG_ENABLE_STATS == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_STATS)) || isRunningReadOnly())) {
		...
		if (_theca != 0) {
			...
			if (!_readOnlyOSCache && !(_theca->ccInitComplete & CC_INIT_COMPLETE)) {
				CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_ERROR, J9NLS_CACHE_BAD_CC_INIT, _theca->ccInitComplete);
			...
			if (isCacheInitComplete() == true) {
				if (_theca->osPageSize != _osPageSize) {
					...
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_OSPAGE_SIZE_MISMATCH);
			...
			if ((true == this->isMemProtectEnabled()) && (false == _readOnlyOSCache)) {
				...
				if ( (mprotectSize != 0) && (_osPageSize > 0)) {
					if (this->setRegionPermissions(PORTLIB, (void *) cacheStart, mprotectSize, (OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE)) != 0) {
						I_32 myerror = omrerror_last_error_number();
						...
						if (doVerbosePages) {
							omrtty_printf("Set initial memory region permissions in SH_CompositeCacheImpl::startup() for entire cache area from %p to %p - for %d bytes\n", (void *) cacheStart, (void *) (cacheStart + mprotectSize), mprotectSize);
						}
					}
				}
			}
			...
			if (!isCacheCorrupt()) {
				...
				if (!checkCacheCRC(cacheHasIntegrity, &crcValue)) {
					CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CC_STARTUP_CACHE_CRC_INVALID, crcValue);
				 ...
				 */
//				if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_FORCE_DUMP_IF_CORRUPT) {
//					TRIGGER_J9HOOK_VM_CORRUPT_CACHE(vm->hookInterface, currentThread);
//				}
			}
			if (!isCacheCorrupt()) {
...
//					if (OMR_ARE_NO_BITS_SET(currentThread->_vm->requiredDebugAttributes, (J9VM_DEBUG_ATTRIBUTE_LINE_NUMBER_TABLE | J9VM_DEBUG_ATTRIBUTE_SOURCE_FILE))) {
//						extraFlags |= OMRSHR_EXTRA_FLAGS_NO_LINE_NUMBERS;
//					}

					if (OMR_ARE_ANY_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED)) {
						if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_DISABLE_BCI)) {
							CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_CACHERETRANSFORMED_IMPLIES_DISABLE_BCI);
						}
						*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_DISABLE_BCI;
					}

					if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_DISABLE_BCI)) {
						extraFlags |= OMRSHR_EXTRA_FLAGS_BCI_ENABLED;
						*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_BCI;
					}

					if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
						/* Creating a new cache with "mprotect=partialpages" option */
						extraFlags |= OMRSHR_EXTRA_FLAGS_MPROTECT_PARTIAL_PAGES;
						if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)) {
							extraFlags |= OMRSHR_EXTRA_FLAGS_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
						}
					}

					if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS)) {
						CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_RESTRICT_CLASSPATHS_ENABLED);
						extraFlags |= OMRSHR_EXTRA_FLAGS_RESTRICT_CLASSPATHS;
					}
...
#if defined(OMRSHR_CACHELET_SUPPORT)
					if (isFirstStart && (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED) && !getContainsCachelets()) {
					...
					_canStoreClasspaths = (false == this->isRestrictClasspathsSet(currentThread)) || OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ALLOW_CLASSPATHS);
					if (_canStoreClasspaths) {
						if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ALLOW_CLASSPATHS)) {
							CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_ALLOW_CLASSPATHS_ENABLED);
						} else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS)) {
							CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING, J9NLS_SHRC_OSCACHE_STARTUP_RESTRICT_CLASSPATHS_IGNORED);
							/* Unset the runtime flag */
							*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS;
						}
					} else {
						CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_CANNOT_STORE_CLASSPATHS);
					}
					...
					if (isMprotectPartialPagesSet(currentThread)) {
						if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
							CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING, J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_ENABLED);
						}
#if defined(J9ZOS390) || defined(AIXPPC)
						*runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES;
#else
						*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES;
#endif /* defined(J9ZOS390) || defined(AIXPPC) */
						if (isMprotectPartialPagesOnStartupSet(currentThread)) {
							if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)) {
								*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
								CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING, J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_ON_STARTUP_ENABLED);
							}
						} else {
							if (OMR_ARE_ANY_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)) {
								*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
								CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_NOT_ENABLED_ON_STARTUP);
							}
						}
					} else {
						if (OMR_ARE_ANY_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND)) {
							/* Unset the runtime flags */
							*_runtimeFlags &= ~(OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND);
							*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
							CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_DISABLED);
						}
					}
				}
...
#if defined (OMRSHR_MSYNC_SUPPORT)
					*_runtimeFlags &= ~(OMRSHR_RUNTIMEFLAG_ENABLE_MSYNC
...
#else
					*_runtimeFlags &= ~(OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT
										| OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL
										| OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
										| OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP
										| OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES
...
#endif 
					if (!_readOnlyOSCache) {
						CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_UNSUPPORTED);
					}
				}

#if defined (OMRSHR_MSYNC_SUPPORT)
				_doHeaderSync = _doReadWriteSync = _doSegmentSync = _doMetaSync = (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MSYNC);
#endif
				if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT)) {
					_doSegmentProtect = !getContainsCachelets();
					_doMetaProtect = true;
				}
				_doHeaderProtect = OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL);
				_doHeaderReadWriteProtect = OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW);
				_doPartialPagesProtect = OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES);
				
				if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_ALLENABLED_INFO);
				}
#if !defined(J9ZOS390) && !defined(AIXPPC)
				else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_INFO1);
				} else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_INFO2);
				} else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_INFO3);
				}
#else
				else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT | OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_INFO2);
...
#endif /* !defined(J9ZOS390) && !defined(AIXPPC) */
				else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT)) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_INFO4);
				} else if (_theca->roundedPagesFlag) {
					CC_TRACE(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_CC_PAGE_PROTECTION_NONE_INFO);
				}
 ...
 */
bool
SH_CompositeCacheImpl::checkCacheCompatibility(OMR_VMThread* currentThread)
{
	bool rc = true;
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
	...
	if (false == this->getIsBCIEnabled()) {
		...
		*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_DISABLE_BCI;
		...
		if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_BCI)) {
			 ...
			 */
			if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_STATS)) {
				*_runtimeFlags &= (~OMRSHR_RUNTIMEFLAG_ENABLE_BCI);
			} else {
	...
	if (true == this->getIsBCIEnabled()) {
		...
		*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_BCI;
		...
		if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_DISABLE_BCI)) {
		...
		if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED)) {
			 ...
			 */
			if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_STATS)) {
				*_runtimeFlags &= (~OMRSHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED);
			} else {
...
SH_CompositeCacheImpl::next(OMR_VMThread* currentThread)
{
	...
	if ((BlockPtr)(*ih) > free) {
		if ((CCITEMLEN(*ih) <= 0) || (CCITEMLEN(*ih) > maxCCItemLen)) {
			 ...
			 */
			OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
			CC_TRACE2(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_CACHE_ENTRY_HEADER_BAD_LEN, ih, CCITEMLEN(*ih));
 ...
 */
UDATA 
SH_CompositeCacheImpl::checkUpdates(OMR_VMThread* currentThread)
{
	IDATA result;
	UDATA returnVal;
 ...
 */
void 
SH_CompositeCacheImpl::doneReadUpdates(OMR_VMThread* currentThread, IDATA updates)
{
	UDATA* updateCountAddress = WSRP_GET(_theca->updateCountPtr, UDATA*);
 ...
 */
SH_CompositeCacheImpl::BlockPtr 
SH_CompositeCacheImpl::nextEntry(OMR_VMThread* currentThread, UDATA* staleItems)
{
	BlockPtr result = 0;
	ShcItemHdr* ih;
 ...
 * 
 * @param [in] currentThread  Point to the OMR_VMThread struct for the current thread
 ...
 */
IDATA 
SH_CompositeCacheImpl::enterWriteMutex(OMR_VMThread* currentThread, bool lockCache, const char* caller)
{
	IDATA rc;
	...
	if (rc == 0) {
		...
		if (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES) {
 ...
 */
IDATA
SH_CompositeCacheImpl::exitWriteMutex(OMR_VMThread* currentThread, const char* caller, bool doDecWriteCounter)
{
	IDATA rc = 0;
 ...
 */
void
SH_CompositeCacheImpl::doLockCache(OMR_VMThread* currentThread)
{
 ...
 */
void
SH_CompositeCacheImpl::doUnlockCache(OMR_VMThread* currentThread)
{
 ...
 *
 * @param [in] currentThread  Points to the OMR_VMThread struct for the current thread
 * 
 * @return true if this VM should use writeHash while loading classes, false otherwise
 */
bool
SH_CompositeCacheImpl::peekForWriteHash(OMR_VMThread *currentThread)
{
	if (!_started || _readOnlyOSCache) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return false;
...
UDATA
SH_CompositeCacheImpl::testAndSetWriteHash(OMR_VMThread *currentThread, UDATA hashValue)
{
...
UDATA
SH_CompositeCacheImpl::tryResetWriteHash(OMR_VMThread *currentThread, UDATA hashValue)
{
	UDATA cacheValue, maskedHash;
 ...
 */
void
SH_CompositeCacheImpl::setWriteHash(OMR_VMThread *currentThread, UDATA hashValue)
{
...
void
SH_CompositeCacheImpl::incReaderCount(OMR_VMThread* currentThread)
{
	UDATA oldNum, value;
...
void
SH_CompositeCacheImpl::decReaderCount(OMR_VMThread* currentThread)
{
	UDATA oldNum, value;
 ...
 */
IDATA
SH_CompositeCacheImpl::enterReadMutex(OMR_VMThread* currentThread, const char* caller)
{
	IDATA rc = 0;
 ...
 */
void
SH_CompositeCacheImpl::exitReadMutex(OMR_VMThread* currentThread, const char* caller)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
 ...
 */
IDATA
SH_CompositeCacheImpl::deleteCache(OMR_VMThread *currentThread, bool suppressVerbose)
{
 ...
 */
void
SH_CompositeCacheImpl::startCriticalUpdate(OMR_VMThread *currentThread)
{
	if (!_started || _readOnlyOSCache) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
 ...
 */
void
SH_CompositeCacheImpl::endCriticalUpdate(OMR_VMThread *currentThread)
{
	if (!_started || _readOnlyOSCache) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateBlock(OMR_VMThread* currentThread, ShcItem* itemToWrite, U_32 align, U_32 alignOffset)
{
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateAOT(OMR_VMThread* currentThread, ShcItem* itemToWrite, U_32 dataBytes)
{
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateJIT(OMR_VMThread* currentThread, ShcItem* itemToWrite, U_32 dataBytes)
{
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateWithSegment(OMR_VMThread* currentThread, ShcItem* itemToWrite, U_32 segmentBufferSize, BlockPtr* segmentBuffer)
{
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateWithReadWriteBlock(OMR_VMThread* currentThread, ShcItem* itemToWrite, U_32 readWriteBufferSize, BlockPtr* readWriteBuffer)
{
...
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocate(OMR_VMThread* currentThread, U_8 type, ShcItem* itemToWrite, U_32 codeDataLen, U_32 separateBufferSize,
		BlockPtr* segmentBuffer, BlockPtr* readWriteBuffer, U_32 align, U_32 alignOffset)
{
	...
	if (type == ALLOCATE_TYPE_BLOCK) {
		...
		regionFullFlag |= OMRSHR_AOT_SPACE_FULL;
		...
		regionFullFlag |= OMRSHR_JIT_SPACE_FULL;
	}
	if (!readWriteBuffer) {
		 ...
		 */
		if (freeBytes > OMRSHR_MIN_GAP_BEFORE_METADATA) {
			freeBytes -= OMRSHR_MIN_GAP_BEFORE_METADATA;
		} else {
			freeBytes = 0;
		}
	...
	if (enoughAvailableSpace) {
		...
		if (ALLOCATE_TYPE_BLOCK == type) {
			/* Similar to the behaviour when setting OMRSHR_BLOCK_SPACE_FULL,
			 * if avalible bytes < CC_MIN_SPACE_BEFORE_CACHE_FULL, OMRSHR_AVAILABLE_SPACE_FULL is set in the previous commit. OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is checked in 
			 * higher level APIs in SH_CacheMap. If it is set, higher level APIs will return direcly and we would not reach here. 
			 * if avalible bytes >= CC_MIN_SPACE_BEFORE_CACHE_FULL, do not set OMRSHR_AVAILABLE_SPACE_FULL here as it is above the threshold.
			 */
			...
			/* If allocating block data, we would reach here only when allocating something that requires more than CC_MIN_SPACE_BEFORE_CACHE_FULL bytes. As free available space
		 	 * is still > CC_MIN_SPACE_BEFORE_CACHE_FULL so OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is not set.
		 	 */
		...
		if (ALLOCATE_TYPE_AOT == type) {
			Trc_SHR_Assert_True((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED)));
			flags |= OMRSHR_AOT_SPACE_FULL;
		} else if (ALLOCATE_TYPE_JIT == type) {
			Trc_SHR_Assert_True((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED)));
			flags |= OMRSHR_JIT_SPACE_FULL;
		} else {
			if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED)) {
				 ...
				 * 		requested amount > freeBlockBytes < (OMRSHR_MIN_GAP_BEFORE_METADATA + OMRSHR_MIN_DUMMY_DATA_SIZE).
				 * In first case we don't want to set OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL as free block bytes is above threshold.
				 * In second case we don't need to set OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL as it would have been done in previous commit.
				 */
				if (freeBlockBytes < (I_32) (OMRSHR_MIN_GAP_BEFORE_METADATA + OMRSHR_MIN_DUMMY_DATA_SIZE)) {
					Trc_SHR_Assert_True(OMRSHR_BLOCK_SPACE_FULL == (OMRSHR_BLOCK_SPACE_FULL & _theca->cacheFullFlags));
				} else {
 ...
 */
SH_CompositeCacheImpl::BlockPtr
SH_CompositeCacheImpl::allocateMetadataEntry(OMR_VMThread* currentThread, BlockPtr allocPtr, ShcItem *itemToWrite, U_32 itemLen)
{
 ...
 * Dummy data is added such that a gap of OMRSHR_MIN_GAP_BEFORE_METADATA bytes is maintained between classes and metadata.
 ...
 */
void
SH_CompositeCacheImpl::fillCacheIfNearlyFull(OMR_VMThread* currentThread)
{
	I_32 freeBlockBytes = getFreeBlockBytes();
	U_32 usedBytes = getUsedBytes();
	U_32 softMaxValue = _theca->softMaxBytes;
	...
	if (freeBlockBytes < (I_32) CC_MIN_SPACE_BEFORE_CACHE_FULL) {
		cacheFullFlags = OMRSHR_BLOCK_SPACE_FULL | OMRSHR_AVAILABLE_SPACE_FULL;
	
		if (freeBlockBytes > (I_32) OMRSHR_MIN_GAP_BEFORE_METADATA) {
			freeBlockBytes -= OMRSHR_MIN_GAP_BEFORE_METADATA;
		} else {
			freeBlockBytes = 0;
		}

		if (freeBlockBytes >= (I_32) OMRSHR_MIN_DUMMY_DATA_SIZE) {
			ShcItem item;
			...
			memset(ITEMDATA(result), OMRSHR_DUMMY_DATA_BYTE, ITEMDATALEN(result));
		...
		cacheFullFlags = OMRSHR_AVAILABLE_SPACE_FULL;
	}

	if (0 != cacheFullFlags) {
		if ((OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL))
			&& (0 == getAvailableReservedAOTBytes(currentThread))
		) {
			cacheFullFlags |= OMRSHR_AOT_SPACE_FULL;
		}

		if ((OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL))
			&& (0 == getAvailableReservedJITBytes(currentThread))
		) {
			cacheFullFlags |= OMRSHR_JIT_SPACE_FULL;
		}
 ...
 */
void
SH_CompositeCacheImpl::rollbackUpdate(OMR_VMThread* currentThread)
{
	if (!_started || _readOnlyOSCache) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
 ...
 */
void
SH_CompositeCacheImpl::commitUpdateHelper(OMR_VMThread* currentThread, bool isCachelet)
{
	OMR_VM *vm = currentThread->_vm;
	UDATA oldNum = 0;
	...
	if (_storedSegmentUsedBytes) {
		...
		if (_doSegmentProtect
		  ...
		  if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP) // || (J9VM_PHASE_NOT_STARTUP == vm->phase)) 
	...
	Trc_SHR_Assert_True((IDATA)(_theca->updateSRP - _theca->segmentSRP) >= (IDATA)OMRSHR_MIN_GAP_BEFORE_METADATA);
 ...
 */
void
SH_CompositeCacheImpl::commitUpdate(OMR_VMThread* currentThread, bool isCachelet)
{
 ...
 */
void
SH_CompositeCacheImpl::markStale(OMR_VMThread* currentThread, BlockPtr blockEnd, bool isCacheLocked)
{
	...
	if (_doMetaProtect && !isCacheLocked) {
		...
		if (setRegionPermissions(_portlib, (void*)areaStart, areaLength, (OMRPORT_PAGE_PROTECT_WRITE | OMRPORT_PAGE_PROTECT_READ)) != 0) {
			OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
			I_32 myerror = omrerror_last_error_number();
	...
	if (_doMetaProtect && !isCacheLocked && ((UDATA)areaStart > (UDATA)_prevScan)) {
		if (setRegionPermissions(_portlib, (void*)areaStart, areaLength, OMRPORT_PAGE_PROTECT_READ) != 0) {
			OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
			I_32 myerror = omrerror_last_error_number();
			Trc_SHR_CC_markStale_setRegionPermissions_Failed(myerror);
			Trc_SHR_Assert_ShouldNeverHappen();
 ...
 */
void
SH_CompositeCacheImpl::findStart(OMR_VMThread* currentThread)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
...
SH_CompositeCacheImpl::getFreeAvailableBytes(void)
{
	...
	if ((U_32)-1 == _theca->softMaxBytes) {
		...
		if (OMR_ARE_NO_BITS_SET(_cacheFullFlags, OMRSHR_AVAILABLE_SPACE_FULL)) {
			ret = (UDATA)(_theca->softMaxBytes - getUsedBytes());
		}
		/* OMRSHR_AVAILABLE_SPACE_FULL is set once (softmx - usedBytes) < CC_MIN_SPACE_BEFORE_CACHE_FULL. (softmx - usedBytes) might be > 0, but no data can be added,
 ...
 */
void
SH_CompositeCacheImpl::runExitCode(OMR_VMThread *currentThread)
{
	SH_OSCache* oscacheToUse = ((_ccHead == NULL) ? _oscache : _ccHead->_oscache);
	...
	if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL)) {
	...
	if ((_commonCCInfo->hasWriteMutexThread == NULL) && (_commonCCInfo->writeMutexID != CC_READONLY_LOCK_VALUE)) {
		...
		if ((lockrc = oscacheToUse->acquireWriteLock(_commonCCInfo->writeMutexID)) == 0) {
			...
			*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_DENY_CACHE_UPDATES;
 ...
 */
IDATA
SH_CompositeCacheImpl::enterReadWriteAreaMutex(OMR_VMThread* currentThread, BOOLEAN readOnly, UDATA* doRebuildLocalData, UDATA* doRebuildCacheData)
{
	IDATA rc = -1;
	SH_OSCache* oscacheToUse = ((_ccHead == NULL) ? _oscache : _ccHead->_oscache);
	...
	if (oscacheToUse && _readWriteAreaBytes) {
		if (_commonCCInfo->readWriteAreaMutexID != CC_READONLY_LOCK_VALUE) {
			...
			if (rc == 0) {
				...
				if ((0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY))) {
					readOnly = TRUE;
				} else if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE)) {
					readOnly = FALSE;
				}
				...
				if (!readOnly) {
					...
					protectHeaderReadWriteArea(currentThread, false);
					if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE)) {
						/* CMVC 176498 : Set conditions to force string table reset with readOnly = TRUE.
						 * This should be executed after OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY case.
						 ...
						 */
						*_runtimeFlags &= (~OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE);
					}
				} else {
					...
					if ((0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY))) {
						...
						*_runtimeFlags &= (~OMRSHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY);
						/* Here variable state is:
						 * 		_theca->readWriteCrashCntr = 1
 ...
 */
IDATA
SH_CompositeCacheImpl::exitReadWriteAreaMutex(OMR_VMThread* currentThread, UDATA resetReason)
{
	IDATA rc = -1;
	SH_OSCache* oscacheToUse = ((_ccHead == NULL) ? _oscache : _ccHead->_oscache);
	UDATA readWriteCrashCntr = UDATA_MAX;
...
UDATA
SH_CompositeCacheImpl::getReaderCount(OMR_VMThread* currentThread)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return 0;
...
bool
SH_CompositeCacheImpl::hasWriteMutex(OMR_VMThread* currentThread)
{
...
bool
SH_CompositeCacheImpl::hasReadWriteMutex(OMR_VMThread* currentThread)
{
	return (_commonCCInfo->hasReadWriteMutexThread == currentThread);
}

void
SH_CompositeCacheImpl::notifyRefreshMutexEntered(OMR_VMThread* currentThread)
{
...
void
SH_CompositeCacheImpl::notifyRefreshMutexExited(OMR_VMThread* currentThread)
{
 ...
 */
void
SH_CompositeCacheImpl::unprotectHeaderReadWriteArea(OMR_VMThread* currentThread, bool changeReadWrite)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
	...
	if (_doHeaderProtect || (_doHeaderReadWriteProtect && changeReadWrite)) {
		...
		if (areaStart != NULL) {
			...
			if ((rc = setRegionPermissions(_portlib, areaStart, areaLength, (OMRPORT_PAGE_PROTECT_WRITE | OMRPORT_PAGE_PROTECT_READ))) != 0) {
				I_32 myerror = omrerror_last_error_number();
			...
			if (isVerbosePages() == true) {
				if (doUnprotectReadWrite) {
					omrtty_printf("Unprotecting cache header and readWrite area - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
				} else {
					omrtty_printf("Unprotecting cache header - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
				}
			}
		}
		if (_doHeaderProtect) {
 ...
 */
void
SH_CompositeCacheImpl::protectHeaderReadWriteArea(OMR_VMThread* currentThread, bool changeReadWrite)
{
	if (!_started) {
		Trc_SHR_Assert_ShouldNeverHappen();
		return;
	...
	if (_doHeaderProtect || (_doHeaderReadWriteProtect && changeReadWrite)) {
		...
		if (areaStart != NULL) {
			...
			if ((rc = setRegionPermissions(_portlib, areaStart, areaLength, OMRPORT_PAGE_PROTECT_READ)) != 0) {
				I_32 myerror = omrerror_last_error_number();
			...
			if (isVerbosePages() == true) {
				if (doProtectReadWrite) {
					omrtty_printf("Protecting cache header and readWrite area - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
				} else {
					omrtty_printf("Protecting cache header - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
				}
			}
		}
		if (_doHeaderProtect) {
...
SH_CompositeCacheImpl::unprotectMetadataArea()
{
	...
	if (_doMetaProtect) {
		...
		if ((rc = setRegionPermissions(_portlib, (void*)areaStart, areaLength, (OMRPORT_PAGE_PROTECT_WRITE | OMRPORT_PAGE_PROTECT_READ))) != 0) {
			I_32 myerror = omrerror_last_error_number();
			Trc_SHR_CC_unprotectMetadataArea_setRegionPermissions_Failed(myerror);
			Trc_SHR_Assert_ShouldNeverHappen();
		}
		if (isVerbosePages() == true) {
			omrtty_printf("Unprotecting entire metadata area - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
		}
...
void
SH_CompositeCacheImpl::protectMetadataArea(OMR_VMThread *currentThread)
{
	if (!_started) {
		return;
	}
	if (_doMetaProtect) {
		...
		if ((true == isCacheMarkedFull(currentThread))
		    || ((OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP))) //|| (J9VM_PHASE_NOT_STARTUP == currentThread->_vm->phase))
		    && (true == _doPartialPagesProtect)
		) {
			areaStart = (BlockPtr)ROUND_DOWN_TO(_osPageSize, (UDATA)areaStart);
		} else {
		...
		areaLength = (U_32)((BlockPtr)CADEBUGSTART(_theca) - (BlockPtr)areaStart);
		if ((rc = setRegionPermissions(_portlib, (void*)areaStart, areaLength, OMRPORT_PAGE_PROTECT_READ)) != 0) {
			I_32 myerror = omrerror_last_error_number();
			Trc_SHR_CC_protectMetadataArea_setRegionPermissions_Failed(myerror);
			Trc_SHR_Assert_ShouldNeverHappen();
		}
		if (isVerbosePages() == true) {
			omrtty_printf("Protecting entire metadata area - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
		}
...
{
	if (_verboseFlags & OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_PAGES) {
		return true;
	}
	return false;
}
...
{
	if (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT)) {
		return true;
	}
	return false;
}
...
SH_CompositeCacheImpl::getCacheAreaCRC(U_8* areaStart, U_32 areaSize)
{
	 ...
	 */
	stepsize = 1535;
	if ((areaSize/stepsize) > OMRSHR_CRC_MAX_SAMPLES) {
		/* Reduce the number of samples */
		stepsize = areaSize/OMRSHR_CRC_MAX_SAMPLES;
	}

	seed = omrcrc32(0, NULL, 0);
	value = omrcrcSparse32(seed, areaStart, areaSize, stepsize);
	...
	if (isCacheInitComplete() == true) {
		...
		if (value && (_theca->crcValid == CC_CRC_VALID_VALUE)) {
			*cacheHasIntegrity = _theca->crcValue == value;
			 ...
			 */
			if ((true == *cacheHasIntegrity) && (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_FAKE_CORRUPTION))) {
				*_runtimeFlags &= ~(OMRSHR_RUNTIMEFLAG_FAKE_CORRUPTION);
				*cacheHasIntegrity = false;
				return false;	
			}
...
//UDATA
//SH_CompositeCacheImpl::getJavacoreData(OMR_VM *vm, J9SharedClassJavacoreDataDescriptor* descriptor)
//{
...
void
SH_CompositeCacheImpl::updateMetadataSegment(OMR_VMThread* currentThread)
{
	OMR_VM* vm = currentThread->_vm;
...
void
SH_CompositeCacheImpl::setContainsCachelets(OMR_VMThread* currentThread)
{
	if (!_started){
		return;
	}
...
/* Note: In case of failure callers are expected to be able to get the error code using portlibrary API omrerror_last_error_number(). */
IDATA
SH_CompositeCacheImpl::setRegionPermissions(OMRPortLibrary* portLibrary, void *address, UDATA length, UDATA flags)
{
#if defined(J9ZTPF)
	return 0;
#else
	...
	if (0 != length) {
		...
		/* Win implementation of omrmmap_protect() fails if length is 0. We don't want to return failure in such case. */
		return 0;
	}
...
SH_CompositeCacheImpl::getIsNoLineNumberEnabled(void)
{
	...
	return ((this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_NO_LINE_NUMBERS) != 0);
}
...
SH_CompositeCacheImpl::getIsNoLineNumberContentEnabled(void)
{
	...
	return (0 != (this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_NO_LINE_NUMBER_CONTENT));
}
 ...
 */
void
SH_CompositeCacheImpl::setNoLineNumberContentEnabled(OMR_VMThread *currentThread)
{
	if(NULL == this->_theca) {
		return;
	}
	setCacheHeaderExtraFlags(currentThread, OMRSHR_EXTRA_FLAGS_NO_LINE_NUMBER_CONTENT);
}
...
SH_CompositeCacheImpl::getIsLineNumberContentEnabled(void)
{
	...
	return (0 != (this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_LINE_NUMBER_CONTENT));
}
...
void
SH_CompositeCacheImpl::setLineNumberContentEnabled(OMR_VMThread* currentThread)
{
	if(NULL == this->_theca) {
		return;
	}
	setCacheHeaderExtraFlags(currentThread, OMRSHR_EXTRA_FLAGS_LINE_NUMBER_CONTENT);
}
...
SH_CompositeCacheImpl::getIsBCIEnabled(void)
{
	...
	return (0 != (this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_BCI_ENABLED));
}
 ...
 */
void
SH_CompositeCacheImpl::setAOTHeaderPresent(OMR_VMThread *currentThread)
{
	Trc_SHR_Assert_True(hasWriteMutex(currentThread));
	setCacheHeaderExtraFlags(currentThread, OMRSHR_EXTRA_FLAGS_AOT_HEADER_PRESENT);
}
 ...
 */
bool
SH_CompositeCacheImpl::isAOTHeaderPresent(OMR_VMThread *currentThread)
{
	Trc_SHR_Assert_True((NULL != this->_theca) && hasWriteMutex(currentThread));
	return (0 != (this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_AOT_HEADER_PRESENT));
}
 ...
 * @param [in] currentThread Pointer to OMR_VMThread structure for the current thread
 *
 * @return 	true if OMRSHR_EXTRA_FLAGS_RESTRICT_CLASSPATHS is set in shared cache header, false otherwise.
 *
 */
bool
SH_CompositeCacheImpl::isRestrictClasspathsSet(OMR_VMThread *currentThread)
{
	Trc_SHR_Assert_True(NULL != this->_theca);
	return (0 != (this->_theca->extraFlags & OMRSHR_EXTRA_FLAGS_RESTRICT_CLASSPATHS));
}

void
SH_CompositeCacheImpl::setCacheHeaderExtraFlags(OMR_VMThread* currentThread, UDATA extraFlags)
{
	Trc_SHR_Assert_True(NULL != this->_theca);
	if (true == _started) {
		unprotectHeaderReadWriteArea(currentThread, false);
...
#if defined(OMRSHR_CACHELETS_SAVE_READWRITE_AREA)
UDATA
SH_CompositeCacheImpl::computeDeployedReadWriteOffsets(OMR_VMThread* currentThread, SH_CompositeCacheImpl* ccHead)
{
 ...
 */
bool
SH_CompositeCacheImpl::copyFromCacheChain(OMR_VMThread* currentThread, SH_CompositeCacheImpl* ccHead, IDATA* metadataOffset)
{
 ...
 */
IDATA
SH_CompositeCacheImpl::fixupSerializedCompiledMethods(OMR_VMThread* currentThread, void* serializedROMClassStartAddress)
{
	IDATA rc = 0;

#if defined(J9VM_INTERP_NATIVE_SUPPORT)
	J9JITConfig *jitConfig = currentThread->_vm->jitConfig;
	...
	while (NULL != (it = (ShcItem*)this->nextEntry(currentThread, NULL))) {
		if (TYPE_COMPILED_METHOD == ITEMTYPE(it)) {
			...
			if (jitConfig->updateROMClassOffsetsInAOTMethod(currentThread->_vm, CMWDATA(wrapper), (J9ROMMethod*)CMWROMMETHOD(wrapper), serializedROMClassStartAddress)) {
				rc = -1;
				break;
			}
		}
 ...
 */
UDATA
SH_CompositeCacheImpl::countROMSegments(OMR_VMThread* currentThread)
{
	UDATA count = 0;
	J9AVLTree* tree = &currentThread->_vm->classMemorySegments->avlTreeData;
 ...
 */
UDATA
SH_CompositeCacheImpl::writeROMSegmentMetadata(OMR_VMThread* currentThread, UDATA numSegments, BlockPtr dest,
 ...
 */
IDATA
SH_CompositeCacheImpl::lockStartupMonitor(OMR_VMThread* currentThread)
{
	if (!_startupMonitor) {
		return -2;
	}
 ...
 */
void
SH_CompositeCacheImpl::unlockStartupMonitor(OMR_VMThread* currentThread)
{
	if (_startupMonitor) {
...
SH_CompositeCacheImpl::setStringTableInitialized(bool isInitialized)
{
	if (!_started) {
		...
		if (isInitialized) {
			_theca->readWriteFlags |= OMRSHR_HEADER_STRING_TABLE_INITIALIZED;
		} else {
			_theca->readWriteFlags &= ~OMRSHR_HEADER_STRING_TABLE_INITIALIZED;
		}
	}
...
SH_CompositeCacheImpl::isStringTableInitialized(void)
{
	...
	return (0 != (_theca->readWriteFlags & OMRSHR_HEADER_STRING_TABLE_INITIALIZED));
}
 ...
 */
//IDATA
//SH_CompositeCacheImpl::allocateClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData, const J9RomClassRequirements * sizes, J9SharedRomClassPieces * pieces)
//{
...
//
//	if (!enoughAvailableSpace) {
//		/* Do not need to set OMRSHR_AVAILABLE_SPACE_FULL here */
//		Trc_SHR_Assert_True((softMaxValue - usedBytes) >= CC_MIN_SPACE_BEFORE_CACHE_FULL);
 ...
 */
//void
//SH_CompositeCacheImpl::rollbackClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData)
//{
 ...
 */
//void
//SH_CompositeCacheImpl::commitClassDebugData(OMR_VMThread* currentThread, U_16 classnameLength, const char* classnameData)
//{
//	bool ok = false;
 ...
 */
IDATA
SH_CompositeCacheImpl::startupForStats(OMR_VMThread* currentThread, SH_OSCache * oscache, U_64 * runtimeFlags, UDATA verboseFlags)
{
	...
	/* _verboseFlags = OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_PAGES; */
			
	if (!oscache->isRunningReadOnly() && _theca->roundedPagesFlag && ((currentThread->_vm->sharedCacheAPI->runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT) != 0)) {
 ...
 */
IDATA
SH_CompositeCacheImpl::shutdownForStats(OMR_VMThread* currentThread)
{
	...
	if (_started == true) {
		...
		if ((*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_MPROTECT) != 0) {
 ...
 */
IDATA 
SH_CompositeCacheImpl::startupNestedForStats(OMR_VMThread* currentThread)
{
	IDATA retval = CC_STARTUP_OK;
 ...
 */
void
SH_CompositeCacheImpl::setRuntimeCacheFullFlags(OMR_VMThread* currentThread)
{
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
	Trc_SHR_Assert_True(hasWriteMutex(currentThread));

	if (0 != (_theca->cacheFullFlags & OMRSHR_ALL_CACHE_FULL_BITS)) {
		...
		if ((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) &&
			(0 != (_theca->cacheFullFlags & OMRSHR_BLOCK_SPACE_FULL))
		) {
			cacheFullFlags |= OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL;
			Trc_SHR_CC_setRuntimeCacheFullFlags_BlockSpaceFull(currentThread);
		}
		if ((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) &&
			(0 != (_theca->cacheFullFlags & OMRSHR_AOT_SPACE_FULL))
		) {
			cacheFullFlags |= OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL;
			Trc_SHR_CC_setRuntimeCacheFullFlags_AOTSpaceFull(currentThread);
		}
		if ((0 == (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) &&
			(0 != (_theca->cacheFullFlags & OMRSHR_JIT_SPACE_FULL))
		) {
			cacheFullFlags |= OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL;
			Trc_SHR_CC_setRuntimeCacheFullFlags_JITSpaceFull(currentThread);
		}
		if ((OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) &&
			(OMR_ARE_ANY_BITS_SET(_theca->cacheFullFlags, OMRSHR_AVAILABLE_SPACE_FULL))
		) {
			cacheFullFlags |= OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL;
		...
		*_runtimeFlags |= cacheFullFlags;
		...
		if (OMR_ARE_ANY_BITS_SET(cacheFullFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
			if ((true == _useWriteHash) && (0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION))) {
				this->setWriteHash(currentThread, 0);
				_reduceStoreContentionDisabled = true;
			}
			*_runtimeFlags &= ~OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION;
			_useWriteHash = false;
		}
		...
		if (0 != cacheFullFlags) {
			if (true == isAllRuntimeCacheFullFlagsSet()) {
			  //				_debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
			...
			} else if (OMR_ARE_ANY_BITS_SET(cacheFullFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
				 ...
				 */
			  //				_debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
			} else if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL| OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
				protectPartiallyFilledPages(currentThread, true, true, true, false);
			} else if (OMR_ARE_ANY_BITS_SET(cacheFullFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
		...
		if (0 != cacheFullFlags) {
			if (true == allRuntimeCacheFullFlagsSet) {
				...
				CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_WARN_FULL_CACHE, _cacheName);
			} else {
				if (0 != (cacheFullFlags & OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
					if (0 != (cacheFullFlags & OMRSHR_RUNTIMEFLAG_ENABLE_NESTED)) {
						/* If we requested a growable cache, this is an error. */
						CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_INFO, J9NLS_SHRC_CM_WARN_FULL_CACHE, _cacheName);
					} else {
						/* Only report Block data space full to std-err with verbose enabled */
						CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_WARN_BLOCK_SPACE_FULL, _cacheName);
					}
				}
				if (OMR_ARE_ALL_BITS_SET(cacheFullFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
					/* Only report cache soft full to std-err with verbose enabled */
					CC_TRACE2(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_AVAILABLE_SPACE_FULL, _cacheName, OPTION_ADJUST_SOFTMX_EQUALS);
				}
				if (0 != (cacheFullFlags & OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
					/* Only report AOT data space full to std-err with verbose enabled */
					CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_AOT_SPACE_FULL, _cacheName);
				}
				if (0 != (cacheFullFlags & OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
					/* Only report JIT data space full to std-err with verbose enabled */
					CC_TRACE1(OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_JIT_SPACE_FULL, _cacheName);
				}
			}
		}
	}
 ...
 */
void
SH_CompositeCacheImpl::protectLastUnusedPages(OMR_VMThread *currentThread)
{
 ...
 */
bool
SH_CompositeCacheImpl::isCacheMarkedFull(OMR_VMThread *currentThread)
{
	Trc_SHR_Assert_True(hasWriteMutex(currentThread));

	return (OMRSHR_ALL_CACHE_FULL_BITS == ((_theca->cacheFullFlags & OMRSHR_ALL_CACHE_FULL_BITS)));
}
 ...
 */
void
SH_CompositeCacheImpl::setCacheHeaderFullFlags(OMR_VMThread *currentThread, UDATA flags, bool setRuntimeFlags)
{
	Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 */
void
SH_CompositeCacheImpl::clearCacheHeaderFullFlags(OMR_VMThread *currentThread)
{
	Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 * These flag include OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL and OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL.
 *
...
{
	if ((0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) &&
		(0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) &&
		(0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) &&
		(0 != (*_runtimeFlags & OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL))
	) {
		return true;
	} else {
		return false;
...
{
	/* don't set DENY_CACHE_UPDATES - we still need updates for timestamp optimizations */
	*_runtimeFlags |= (OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL | OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL | OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
 ...
 */
I_32
SH_CompositeCacheImpl::getAvailableReservedAOTBytes(OMR_VMThread *currentThread)
{
	I_32 minAOT = _theca->minAOT;
	I_32 aotBytes = (I_32)_theca->aotBytes;
 ...
 */
I_32
SH_CompositeCacheImpl::getAvailableReservedJITBytes(OMR_VMThread *currentThread)
{
	I_32 minJIT = _theca->minJIT;
	I_32 jitBytes = (I_32)_theca->jitBytes;
 ...
 */
I_32
SH_CompositeCacheImpl::getFreeAOTBytes(OMR_VMThread *currentThread)
{
 ...
 */
I_32
SH_CompositeCacheImpl::getFreeJITBytes(OMR_VMThread *currentThread)
{
 ...
 */
IDATA
SH_CompositeCacheImpl::restoreFromSnapshot(OMR_VM* vm, const char* cacheName, bool* cacheExist)
{
 ...
 */
void
SH_CompositeCacheImpl::dontNeedMetadata(OMR_VMThread *currentThread, const void* startAddress, size_t length) {
 ...
 */
void
SH_CompositeCacheImpl::changePartialPageProtection(OMR_VMThread *currentThread, void *addr, bool readOnly, bool phaseCheck)
{
	IDATA rc = 0;
	BlockPtr pageAddr = NULL;
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);

	Trc_SHR_CC_changePartialPageProtection_Entry(currentThread, addr, readOnly, phaseCheck, NULL); //, currentThread->_vm->phase);
	...
	if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)
	        || (false == phaseCheck) //|| (J9VM_PHASE_NOT_STARTUP == currentThread->_vm->phase)))
		&& (true == _doPartialPagesProtect)
	) {
		...
		rc = setRegionPermissions(_portlib, (void *)pageAddr, _osPageSize, readOnly ? OMRPORT_PAGE_PROTECT_READ : OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
		if (0 != rc) {
			I_32 myerror = omrerror_last_error_number();
 ...
 */
void
SH_CompositeCacheImpl::protectPartiallyFilledPages(OMR_VMThread *currentThread,  bool protectSegmentPage, bool protectMetadataPage, bool protectDebugDataPages, bool phaseCheck)
{
	if (0 != _osPageSize) {
 ...
 */
void
SH_CompositeCacheImpl::unprotectPartiallyFilledPages(OMR_VMThread *currentThread,  bool unprotectSegmentPage, bool unprotectMetadataPage, bool unprotectDebugDataPages, bool phaseCheck)
{
 ...
 */
void
SH_CompositeCacheImpl::setSoftMaxBytes(OMR_VMThread *currentThread, U_32 softMaxBytes, bool isJCLCall)
{
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
	/* add trace points */
	Trc_SHR_Assert_True(
 ...
 */
I_32
SH_CompositeCacheImpl::tryAdjustMinMaxSizes(OMR_VMThread *currentThread, bool isJCLCall)
{
	...
	if (adjustMinAOT || adjustMinJIT) {
		...
		U_32 maxUsedBytes = (maxLimit < (totalBytes - freeDebugBytes - OMRSHR_MIN_GAP_BEFORE_METADATA)) ? maxLimit : (totalBytes - freeDebugBytes - OMRSHR_MIN_GAP_BEFORE_METADATA);
		 ...
		 * Leave at leaset OMRSHR_MIN_GAP_BEFORE_METADATA, so OMRSHR_MIN_GAP_BEFORE_METADATA should also be subtracted.
		 */
	...
	if (adjustMaxAOT) {
		if (maxAOT != _theca->maxAOT) {
			if ((_theca->maxAOT >= 0)
				...
				flagsToUnset |= OMRSHR_AOT_SPACE_FULL;
			}
			_theca->maxAOT = maxAOT;
			ret |= OMRSHR_MAX_AOT_ADJUSTED;
	...
	if (adjustMaxJIT) {
		if (maxJIT != _theca->maxJIT) {
			if ((_theca->maxJIT >= 0)
				...
				flagsToUnset |= OMRSHR_JIT_SPACE_FULL;
			}
			_theca->maxJIT = maxJIT;
			ret |= OMRSHR_MAX_JIT_ADJUSTED;
	...
	if (adjustSoftMax) {
		if (softMax != _theca->softMaxBytes) {
			if (softMax > _theca->softMaxBytes) {
				...
				flagsToUnset |= OMRSHR_AVAILABLE_SPACE_FULL;
				flagsToUnset |= OMRSHR_BLOCK_SPACE_FULL;
				flagsToUnset |= OMRSHR_JIT_SPACE_FULL;
				flagsToUnset |= OMRSHR_AOT_SPACE_FULL;
			}
			setSoftMaxBytes(currentThread, softMax, isJCLCall);
			ret |= OMRSHR_SOFTMX_ADJUSTED;
		}
	}
 ...
 */
void
SH_CompositeCacheImpl::updateRuntimeFullFlags(OMR_VMThread *currentThread)
{
	...
	if (_readOnlyOSCache || OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_ENABLE_READONLY)) {
		goto done;
	}
	...
	if (OMR_ARE_NO_BITS_SET(cacheFullFlags, OMRSHR_BLOCK_SPACE_FULL)) {
		if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagUnset(currentThread, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL);
			flagUnset |= OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL;
		}
	} else {
		if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagSet(currentThread, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL);
			flagSet |= OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL;
		}
	}

	if (OMR_ARE_NO_BITS_SET(cacheFullFlags, OMRSHR_AVAILABLE_SPACE_FULL)) {
		if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
			if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
				/* OMRSHR_BLOCK_SPACE_FULL is always unset together with OMRSHR_AVAILABLE_SPACE_FULL, do not need to remove J9AVLTREE_DISABLE_SHARED_TREE_UPDATES
				 * when unseting OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL, do it here is enough.
				 */
				if (NULL != currentThread->_vm->sharedInvariantInternTable) {
					Trc_SHR_CC_updateRuntimeFullFlags_flagUnset(currentThread, J9AVLTREE_DISABLE_SHARED_TREE_UPDATES);
					currentThread->_vm->sharedInvariantInternTable->flags &= ~J9AVLTREE_DISABLE_SHARED_TREE_UPDATES;
				}
				if (_reduceStoreContentionDisabled) {
					Trc_SHR_CC_updateRuntimeFullFlags_flagSet(currentThread, OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION);
					*_runtimeFlags |= OMRSHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION;
					_useWriteHash = true;
					_reduceStoreContentionDisabled = false;
				}
			}
			Trc_SHR_CC_updateRuntimeFullFlags_flagUnset(currentThread, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
			flagUnset |= OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL;
			resetSoftmxUnstoredBytes = true;
		}
	} else {
		if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagSet(currentThread, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
			flagSet |= OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL;
	...
	if (OMR_ARE_NO_BITS_SET(cacheFullFlags, OMRSHR_AOT_SPACE_FULL)) {
		if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagUnset(currentThread, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL);
			flagUnset |= OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL;
			/* OMRSHR_AOT_SPACE_FULL can be removed when either softmx or maxAOT gets increased by this VM or another VM. Keep a local copy of _maxAOT,
			 * so that we know whether it is the increased maxAOT that removes OMRSHR_AOT_SPACE_FULL. Set _maxAOTUnstoredBytes to 0 only when maxAOT is increased.
			 */
		...
		if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagSet(currentThread, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL);
			flagSet |= OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL;
		}
	}

	if (OMR_ARE_NO_BITS_SET(cacheFullFlags, OMRSHR_JIT_SPACE_FULL)) {
		if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagUnset(currentThread, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL);
			flagUnset |= OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL;
			/* OMRSHR_JIT_SPACE_FULL can be removed when either softmx or maxJIT gets increased by this VM or another VM. Keep a local copy of _maxJIT,
			 * so that we know whether it is the increased maxJIT that removes OMRSHR_JIT_SPACE_FULL. Set _maxJITUnstoredBytes to 0 only when maxJIT is increased.
			 */
		...
		if (OMR_ARE_NO_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
			Trc_SHR_CC_updateRuntimeFullFlags_flagSet(currentThread, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL);
			flagSet |= OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL;
		}
	}
	...
	if ((holdWriteMutex)
		...
		if (flagSet > 0) {
			if (true == isAllRuntimeCacheFullFlagsSet()) {
			  //				_debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
				protectLastUnusedPages(currentThread);
			} else if (OMR_ARE_ANY_BITS_SET(flagSet, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
				/* At this point no more ROMClass can be added.
				 * This implies no new data can be added in class debug area.
				 * Partially filled page and unused pages in class debug area can now be protected.
				 */
			  //				_debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
				...
				protectPartiallyFilledPages(currentThread, true, true, true, false);
			} else if (OMR_ARE_ANY_BITS_SET(flagSet, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
				/* Only protect partially filled pages in debug area */
				protectPartiallyFilledPages(currentThread, false, false, true, false);
			}
		}
		...
		if (flagUnset > 0) {
			if (OMR_ARE_ANY_BITS_SET(flagUnset, OMRSHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
			...
			} else if (OMR_ARE_ANY_BITS_SET(flagUnset, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
				unprotectPartiallyFilledPages(currentThread, true, true, true, false);
			} else if (OMR_ARE_ANY_BITS_SET(flagUnset, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL | OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
 ...
 */
void
SH_CompositeCacheImpl::unsetCacheHeaderFullFlags(OMR_VMThread *currentThread, UDATA flagsToUnset)
{
...
SH_CompositeCacheImpl::increaseUnstoredBytes(U_32 blockBytes, U_32 aotBytes, U_32 jitBytes)
{
	...
	if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
		...
		if (aotBytes > 0) {
			if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
				/* Add check for OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL as maxAOT or softmx might have been increased and OMRSHR_RUNTIMEFLAG_AOT_SPACE_FULL is removed.
				 * No need to increase _maxAOTUnstoredBytes or _softmxUnstoredBytes if that's the case.
				 */
				if (-1 == maxAOT) {
					/* maxAOT is not set. It is the softmx that prevents the AOT data being stored into the cache. OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL
		...
		if (jitBytes > 0) {
			if (OMR_ARE_ALL_BITS_SET(*_runtimeFlags, OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
				/* Add check for OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL as maxJIT or softmx might have been increased and OMRSHR_RUNTIMEFLAG_JIT_SPACE_FULL is removed.
				 * No need to increase _maxJITUnstoredBytes or _softmxUnstoredBytes if that's the case.
				 */
				if (-1 == maxJIT) {
					/* maxJIT is not set. It is the softmx that prevents the JIT data being stored into the cache. OMRSHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL
					 * is not set as free available space can still be > CC_MIN_SPACE_BEFORE_CACHE_FULL.
					 */
