...
/*
 * Macros used by omrshr_destroy_cache to specify cache status.
 * Except J9SH_DESTROYED_OLDER_GEN_CACHE, rest are also used to specify return value for omrshr_destroy_cache.
 */
...
IDATA omrshr_destroy_snapshot(struct OMR_VM* vm, const char* ctrlDirName, UDATA verboseFlags, const char* cacheName, UDATA generationStart, UDATA generationEnd, J9PortShcVersion* versionData);

IDATA omrshr_destroy_all_snapshot(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags);

IDATA omrshr_list_caches(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags);

IDATA omrshr_destroy_all_cache(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags);

IDATA omrshr_destroy_expire_cache(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags, UDATA minutes);

IDATA omrshr_stat_cache(struct OMR_VM* vm, const char* cacheDirName, UDATA verboseFlags, const char* name, J9PortShcVersion* versionData, UDATA generation);

IDATA omrshr_report_utility_incompatible(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags, const char* name, const char* utility);
