...
void
SH_Manager::HashLinkedListImpl::initialize(const OMRUTF8* key_, const ShcItem* item_, SH_CompositeCache* cachelet_, UDATA hashValue_)
{
	Trc_SHR_M_HashLinkedListImpl_initialize_Entry();

	_key = key_? (U_8*)OMRUTF8_DATA(key_): NULL;
	_keySize = key_? (U_16)OMRUTF8_LENGTH(key_): 0;
	_item = item_;
...
SH_Manager::createLink(const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet, UDATA hashPrimeValue, const J9Pool* allocationPool)
{
	HashLinkedListImpl *newLink, *memPtr;

	Trc_SHR_Assert_True(key != NULL);
	Trc_SHR_M_HashLinkedListImpl_createLink_Entry(OMRUTF8_LENGTH(key), OMRUTF8_DATA(key), item);
...
void
SH_Manager::tearDownHashTable(OMR_VMThread* currentThread)
{
...
IDATA
SH_Manager::startup(OMR_VMThread* currentThread, U_64* runtimeFlags_, UDATA verboseFlags_, UDATA cacheSizeBytes)
{
	UDATA actualState;
...
void
SH_Manager::shutDown(OMR_VMThread* currentThread)
{
...
/* THREADING: Must only be called single-threaded and not concurrent with any other functions */
void
SH_Manager::cleanup(OMR_VMThread* currentThread)
{
...
/* THREADING: Must only be called single-threaded and not concurrent with any other functions */
IDATA 
SH_Manager::reset(OMR_VMThread* currentThread)
{
	IDATA returnVal = 0;
 ...
 */
SH_Manager::HashLinkedListImpl* 
SH_Manager::hllTableAdd(OMR_VMThread* currentThread, const J9Pool* linkPool, const OMRUTF8* key, const ShcItem* item, UDATA hashPrimeValue, SH_CompositeCache* cachelet, HashLinkedListImpl** addToList)
{
 ...
 */
SH_Manager::HashLinkedListImpl* 
SH_Manager::hllTableLookup(OMR_VMThread* currentThread, const char* name, U_16 nameLen, bool allowCacheletStartup)
{
 ...
 */
SH_Manager::HashLinkedListImpl*
SH_Manager::hllTableLookupHelper(OMR_VMThread* currentThread, U_8* key, U_16 keySize, UDATA hashValue, SH_CompositeCache* cachelet)
{
...
IDATA
SH_Manager::startupHintCachelets(OMR_VMThread* currentThread, UDATA hint)
{
...
SH_Manager::hllTableUpdate(OMR_VMThread* currentThread, const J9Pool* linkPool, const OMRUTF8* key, const ShcItem* item, SH_CompositeCache* cachelet)
{
	...
	Trc_SHR_M_hllTableUpdate_Entry(currentThread, OMRUTF8_LENGTH(key), OMRUTF8_DATA(key), item);
 ...
 */
bool
SH_Manager::lockHashTable(OMR_VMThread* currentThread, const char* funcName)
{
...
SH_Manager::hllHashEqualFn(void* left, void* right, void *userData)
{
	...
	result = OMRUTF8_DATA_EQUALS(leftItem->_key, leftItem->_keySize, rightItem->_key, rightItem->_keySize);
 ...
 */
void 
SH_Manager::getNumItems(OMR_VMThread* currentThread, UDATA* nonStaleItems, UDATA* staleItems)
{
