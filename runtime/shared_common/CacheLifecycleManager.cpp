...
#define CLM_TRACE(var1) if (verboseFlags) omrnls_printf(J9NLS_INFO, var1)
#define CLM_TRACE1(var1, p1) if (verboseFlags) omrnls_printf(J9NLS_INFO, var1, p1)
#define CLM_TRACE2(var1, p1, p2) if (verboseFlags) omrnls_printf(J9NLS_INFO, var1, p1, p2)
#define CLM_TRACE_NOTAG(var1) if (verboseFlags) omrnls_printf(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1)
#define CLM_TRACE1_NOTAG(var1, p1) if (verboseFlags) omrnls_printf(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1, p1)
#define CLM_TRACE2_NOTAG(var1, p1, p2) if (verboseFlags) omrnls_printf(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1, p1, p2)
#define CLM_ERR_TRACE(var1) if (verboseFlags) omrnls_printf(J9NLS_ERROR, var1)
#define CLM_ERR_TRACE1(var1, p1) if (verboseFlags) omrnls_printf(J9NLS_ERROR, var1, p1)
...
static IDATA deleteSnapshot(struct OMR_VM* vm, UDATA verboseFlags, const char* pathFileName);
static J9Pool* getCacheList(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, bool includeOldGenerations, UDATA reason);
 ...
 *
 * @param [in] vm  pointer to OMR_VM structure.
 ...
 */
IDATA
j9shr_iterateSharedCaches(OMR_VM *vm,	const char *ctrlDirName, UDATA groupPerm, BOOLEAN useCommandLineValue,
 		IDATA (*callback)(OMR_VM *vm, J9SharedCacheInfo *event_data, void *user_data),
 		void *user_data)
{
	...
	if (useCommandLineValue == TRUE){
		UDATA vmGroupPerm = 0;
		if (vm->sharedCacheAPI->runtimeFlags & OMRSHR_RUNTIMEFLAG_ENABLE_GROUP_ACCESS) {
			vmGroupPerm = 1;
		}
...
//		snapshotList = SH_OSCache::getAllCacheStatistics(vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), false, false, SHR_STATS_REASON_ITERATE, false);
	...
	if (!noCacheExist) {
		...
		while (cacheInfo) {
			...
			if ((callbackData.isCompatible == 1) && (cacheInfo->isJavaCorePopulated == 1)) {
			  callbackData.cacheSize = 0; // cacheInfo->javacoreData.cacheSize;
			  callbackData.freeBytes = 0; // cacheInfo->javacoreData.freeBytes;
			  callbackData.softMaxBytes =  0; // cacheInfo->javacoreData.softMaxBytes;
			} else {
			...
			if (OMR_ARE_ALL_BITS_SET(cacheInfo->versionData.feature, OMRSH_FEATURE_COMPRESSED_POINTERS)) {
				Trc_SHR_Assert_True(OMRSH_ADDRMODE_32 != callbackData.addrMode);
	...
	if (!noSnapshotExist) {
		...
		while (cacheInfo) {
			...
			callbackData.cacheSize = (UDATA)OMRSH_OSCACHE_UNKNOWN;
			callbackData.softMaxBytes = (UDATA)OMRSH_OSCACHE_UNKNOWN;
			callbackData.freeBytes = (UDATA)OMRSH_OSCACHE_UNKNOWN;
...
printSharedCache(void* element, void* param)
{
	...
	UDATA verboseFlags = OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE;
	...
	if (((state->printCompatibleCache == true) && (currentItem->isCompatible))
		...
		OMRPORT_ACCESS_FROM_VMC(state->vm);

		Trc_SHR_CLM_printSharedCache_Entry();

		if (state->printIntro) {
			char cacheDir[OMRSH_MAXPATH];

			SH_OSCache::getCacheDir(state->vm, state->ctrlDirName, cacheDir, OMRSH_MAXPATH, J9PORT_SHR_CACHE_TYPE_PERSISTENT);
		...
		if (OMR_ARE_ALL_BITS_SET(currentItem->versionData.feature, OMRSH_FEATURE_COMPRESSED_POINTERS)) {
			j9tty_printf(PORTLIB, "%-16s", "cr");
		} else if (OMR_ARE_ALL_BITS_SET(currentItem->versionData.feature, OMRSH_FEATURE_NON_COMPRESSED_POINTERS)) {
			j9tty_printf(PORTLIB, "%-16s", "non-cr");
		} else {
			j9tty_printf(PORTLIB, "%-16s", "default");
		}
#if !defined(WIN32)
		if (currentItem->os_shmid != (UDATA)OMRSH_OSCACHE_UNKNOWN) {
			j9tty_printf(PORTLIB, "%-15d", currentItem->os_shmid);
		} else {
			j9tty_printf(PORTLIB, "%-15s", "");
		}
		if (currentItem->os_semid != (UDATA)OMRSH_OSCACHE_UNKNOWN) {
		...
		if (currentItem->nattach == 0) {
			if (OMRSH_OSCACHE_UNKNOWN == currentItem->lastdetach) {
				j9tty_printf(PORTLIB, "%-15s\n", "Unknown");
			} else {
				time_t t;
		...
		} else if ((currentItem->nattach == OMRSH_OSCACHE_UNKNOWN) || (currentItem->lastdetach == OMRSH_OSCACHE_UNKNOWN)) {
			if (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == currentItem->versionData.cacheType) {
				/* no last detach time for snapshot,  currentItem->nattach and currentItem->lastdetach are both OMRSH_OSCACHE_UNKNOWN */
				j9tty_printf(PORTLIB,"\n");
			} else {
				j9tty_printf(PORTLIB, "%-15s\n", "Unknown");
			}
...
deleteSharedCache(void *element, void* param)
{
	...
	if ((OMRSH_DESTROY_FAILED_CURRENT_GEN_CACHE == returnVal) ||
		(OMRSH_DESTROY_FAILED_OLDER_GEN_CACHE == returnVal) ||
		(OMRSH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT == returnVal) ||
		(OMRSH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT == returnVal) ||
		(OMRSH_DESTROYED_NONE == returnVal )) {
		state->result = -1;
...
static J9Pool*
getCacheList(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, bool includeOldGenerations, UDATA reason)
{
	J9Pool* cacheList;
 ...
 * It is the responsibility of the caller to free the pool created */
static J9Pool*
findIncompatibleCachesForName(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, const char* name)
{
	J9Pool* cacheList;
 ...
 */
IDATA 
j9shr_list_caches(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags)
{
 ...
 */
IDATA 
j9shr_destroy_expire_cache(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags, UDATA minutes) {
	J9SharedCacheManagerParm param;
	J9Pool* cacheList;
 ...
 */
IDATA 
j9shr_destroy_all_cache(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags) {
	J9SharedCacheManagerParm param;
	J9Pool* cacheList;
	char cacheDir[OMRSH_MAXPATH];
	...
	SH_OSCache::getCacheDir(vm, ctrlDirName, cacheDir, OMRSH_MAXPATH, J9PORT_SHR_CACHE_TYPE_PERSISTENT);
	j9tty_printf(PORTLIB, "\n");
 ...
 */
IDATA
j9shr_destroy_all_snapshot(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags)
{
	J9SharedCacheManagerParm param;
	J9Pool* snapshotList = NULL;
	char cacheDir[OMRSH_MAXPATH];
	...
	if (-1 == SH_OSCache::getCacheDir(vm, ctrlDirName, cacheDir, OMRSH_MAXPATH, J9PORT_SHR_CACHE_TYPE_SNAPSHOT)) {
 ...
 *
 * @returns	OMRSH_DESTROYED_ALL_CACHE/OMRSH_DESTROYED_ALL_SNAPSHOT when no cache/snapshot exists or successfully destroyed all caches/snapshots,
 * 			OMRSH_DESTROYED_NONE when it fails to destroy any cache/snapshot,
 * 			OMRSH_DESTROY_FAILED_CURRENT_GEN_CACHE/OMRSH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT when it failed to destroy cache/snapshot of current generation,
 * 			OMRSH_DESTROY_FAILED_OLDER_GEN_CACHE/OMRSH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT when it failed to destroy one or more older generation cache/snapshot and either current generation cache/snapshot does not exists or is successfully destroyed.
 *
 * Note: cacheType = 0 is an indicator to use platform default value for cacheType.
 */
IDATA
j9shr_destroySharedCache(OMR_VM *vm, const char *ctrlDirName, const char *name, U_32 cacheType, BOOLEAN useCommandLineValues)
{
	...
	if (UnitTest::SHAREDCACHE_API_TEST == UnitTest::unitTest) {
		verboseFlags = OMRSHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT;
	}
	if (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == cacheType) {
 ...
 *
 * @returns	OMRSH_DESTROYED_ALL_CACHE when no cache exists or successfully destroyed all caches,
 * 			OMRSH_DESTROYED_NONE when it fails to destroy any cache,
 * 			OMRSH_DESTROY_FAILED_CURRENT_GEN_CACHE when it failed to destroy cache of current generation,
 * 			OMRSH_DESTROY_FAILED_OLDER_GEN_CACHE when it failed to destroy one or more older generation cache and either current generation cache does not exists or is successfully destroyed.
 */
IDATA
j9shr_destroy_cache(struct OMR_VM* vm, const char* ctrlDirName, UDATA verboseFlags, const char* cacheName, UDATA generationStart, UDATA generationEnd, J9PortShcVersion* versionData, BOOLEAN isReset) {
	SH_OSCache* oscache;
	IDATA returnVal = OMRSH_DESTROYED_ALL_CACHE;
	IDATA cacheStatus = OMRSH_DESTROYED_NONE;
	...
	for (UDATA i = generationStart; i <= lastGeneration; i++) {
		...
		if (1 == j9shr_stat_cache(vm, cacheDirName, 0 , cacheName, versionData, i)) {
			cacheStatus = OMRSH_DESTROYED_OLDER_GEN_CACHE;
			...
			if (!oscache->startup(vm, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, cacheName, vm->sharedClassPreinitConfig, 0, OMRSH_OSCACHE_OPEXIST_DESTROY, verboseFlags, 0/*runtime check*/, 0, vm->sharedCacheAPI->storageKeyTesting, versionData, NULL, SHR_STARTUP_REASON_DESTROY)) {
				if (oscache->getError() != OMRSH_OSCACHE_NO_CACHE) {
					cacheStatus = OMRSH_DESTROY_FAILED_OLDER_GEN_CACHE;
					noCacheExists = false;
				}
			} else {
				noCacheExists = false;
	...
	if (false == noCacheExists) {
		if (OMRSH_DESTROYED_OLDER_GEN_CACHE == cacheStatus) {
	...
	if (OSCACHE_CURRENT_CACHE_GEN == generationEnd) {
		...
		if (1 == j9shr_stat_cache(vm, cacheDirName, 0 , cacheName, versionData, OSCACHE_CURRENT_CACHE_GEN)) {
			...
			if (!oscache->startup(vm, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, cacheName, vm->sharedClassPreinitConfig, 0, OMRSH_OSCACHE_OPEXIST_DESTROY, verboseFlags, 0/*runtime check*/, 0, vm->sharedCacheAPI->storageKeyTesting, versionData, NULL, SHR_STARTUP_REASON_DESTROY)) {
				...
				if (oscache->getError() != OMRSH_OSCACHE_NO_CACHE) {
					noCacheExists = false;
					returnVal = OMRSH_DESTROY_FAILED_CURRENT_GEN_CACHE;
...
static IDATA
deleteSnapshot(struct OMR_VM* vm, UDATA verboseFlags, const char* pathFileName)
{
	OMRPORT_ACCESS_FROM_VMC(vm);
	I_32 fileRc = -1;
	I_64 fileSize = 0;
 ...
 *
 * @return	OMRSH_DESTROYED_ALL_SNAPSHOT when no snapshot exists or successfully destroyed all snapshots,
 * 			OMRSH_DESTROYED_NONE when it fails to destroy any snapshots,
 * 			OMRSH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT when it failed to destroy snapshot of cache of current generation,
 * 			OMRSH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT when it failed to destroy one or more snapshot of older generation cache 
 * 													and either snapshot of current generation cache does not exists or is successfully destroyed.
 */
IDATA
j9shr_destroy_snapshot(struct OMR_VM* vm, const char* ctrlDirName, UDATA verboseFlags, const char* snapshotName, UDATA generationStart, UDATA generationEnd, J9PortShcVersion* versionData)
{
	IDATA returnVal = OMRSH_DESTROYED_ALL_SNAPSHOT;
	IDATA snapshotStatus = OMRSH_DESTROYED_NONE;
	...
	if (-1 == SH_OSCache::getCacheDir(vm, ctrlDirName, cacheDirName, OMRSH_MAXPATH, J9PORT_SHR_CACHE_TYPE_SNAPSHOT)) {
		Trc_SHR_CLM_j9shr_destroy_snapshot_getCacheDirFailed();
		CLM_TRACE(J9NLS_SHRC_GETSNAPSHOTDIR_FAILED);
		returnVal = OMRSH_DESTROYED_NONE;
		goto done;
	}
	...
	for (UDATA i = generationStart; i <= lastGeneration; i++) {
		...
		SH_OSCache::getCachePathName(PORTLIB, cacheDirName, pathFileName, OMRSH_MAXPATH, nameWithVGen);
		if (EsIsFile == j9file_attr(pathFileName)) {
			IDATA rc = deleteSnapshot(vm, verboseFlags, pathFileName);

			if (0 == rc) {
				noSnapshotExists = false;
				snapshotStatus = OMRSH_DESTROYED_OLDER_GEN_SNAPSHOT;
				CLM_TRACE1(J9NLS_SHRC_CLCM_SNAPSHOT_REMOVED_OLDER_GEN, snapshotName);
			} else if (-2 == rc) {
				noSnapshotExists = false;
				snapshotStatus = OMRSH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT;
	...
	if (OSCACHE_CURRENT_CACHE_GEN == generationEnd) {
		...
		if (EsIsFile == j9file_attr(pathFileName)) {
			...
			if (0 == rc) {
				...
				noSnapshotExists = false;
				returnVal = OMRSH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT;
 ...
 */	
IDATA
j9shr_stat_cache(struct OMR_VM* vm, const char* cacheDirName, UDATA verboseFlags, const char* name, J9PortShcVersion* versionData, UDATA generation)
{
	char nameWithVGen[CACHE_ROOT_MAXLEN];
	IDATA result;
 ...
 */
IDATA
j9shr_report_utility_incompatible(struct OMR_VM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags, const char* name, const char* utility)
{
	J9Pool* list;
	OMRPORT_ACCESS_FROM_VMC(vm);
	IDATA rc = 0;
