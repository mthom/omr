...
J9HashTable* 
SH_ScopeManagerImpl::localHashTableCreate(OMR_VMThread* currentThread, U_32 initialEntries)
{
	J9HashTable* returnVal;

	Trc_SHR_SMI_localHashTableCreate_Entry(currentThread, initialEntries);
	returnVal = hashTableNew(_portlib, OMR_GET_CALLSITE(), initialEntries, sizeof(HashEntry), sizeof(char *), 0, OMRMEM_CATEGORY_CLASSES, SH_ScopeManagerImpl::scHashFn, SH_ScopeManagerImpl::scHashEqualFn, NULL, NULL); // (void*)currentThread->javaVM->internalVMFunctions);
	Trc_SHR_SMI_localHashTableCreate_Exit(currentThread, returnVal);
	return returnVal;
}

IDATA
SH_ScopeManagerImpl::localInitializePools(OMR_VMThread* currentThread)
{
	return 0;
}

void
SH_ScopeManagerImpl::localTearDownPools(OMR_VMThread* currentThread)
{
}
...
SH_ScopeManagerImpl::scHashFn(void* item, void *userData)
{
	...
	const OMRUTF8* utf8 = entryItem->_value;
	//	J9InternalVMFunctions* internalFunctionTable = (J9InternalVMFunctions*)userData;
	UDATA hashValue = 0;

	Trc_SHR_SMI_scHashFn_Entry(item);
	hashValue = 0; //internalFunctionTable->computeHashForUTF8(OMRUTF8_DATA(utf8), OMRUTF8_LENGTH(utf8));
...
SH_ScopeManagerImpl::scHashEqualFn(void* left, void* right, void *userData)
{
	...
	const OMRUTF8* utf8left = leftItem->_value;
	const OMRUTF8* utf8right = rightItem->_value;
	UDATA result;
	...
	if (OMRUTF8_LENGTH(utf8left) != OMRUTF8_LENGTH(utf8right)) {
		Trc_SHR_SMI_scHashEqualFn_Exit2();
		return 0;
	}
	if (OMRUTF8_DATA(utf8left)==NULL || OMRUTF8_DATA(utf8right)==NULL) {
		Trc_SHR_SMI_scHashEqualFn_Exit1();
		return 0;
	}
	result = OMRUTF8_EQUALS(utf8left, utf8right);
	Trc_SHR_SMI_scHashEqualFn_Exit3(result);
	return result;
}

const SH_ScopeManagerImpl::HashEntry* 
SH_ScopeManagerImpl::scTableAdd(OMR_VMThread* currentThread, const ShcItem* item, SH_CompositeCache* cachelet)
{
	const OMRUTF8* key = (const OMRUTF8*)ITEMDATA(item);
	const HashEntry* returnVal = NULL;
	HashEntry entry(key, cachelet);
	OMRPORT_ACCESS_FROM_OMRPORT(_portlib);
	
	Trc_SHR_SMI_scTableAdd_Entry(currentThread, OMRUTF8_LENGTH(key), OMRUTF8_DATA(key), item);
...
const OMRUTF8*
SH_ScopeManagerImpl::scTableLookup(OMR_VMThread* currentThread, const OMRUTF8* key)
{
	const HashEntry* found = NULL;
	const OMRUTF8* returnVal = NULL;
	HashEntry searchEntry(key, NULL);
	
	Trc_SHR_SMI_scTableLookup_Entry(currentThread, OMRUTF8_LENGTH(key), OMRUTF8_DATA(key));
 ...
 */
IDATA
SH_ScopeManagerImpl::scCollectHashes(OMR_VMThread* currentThread, SH_CompositeCache* cachelet, CacheletHints* hints)
{
 ...
 */	
SH_ScopeManagerImpl*
SH_ScopeManagerImpl::newInstance(OMR_VM* vm, SH_SharedCache* cache_, SH_ScopeManagerImpl* memForConstructor)
{
...
void
SH_ScopeManagerImpl::initialize(OMR_VM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
 ...
 */
bool 
SH_ScopeManagerImpl::storeNew(OMR_VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
...
/**
 * Find a cached scope for a given OMRUTF8 local scope
 *
 * @param[in] currentThread  The current VM thread
 * @param[in] localScope  A local OMRUTF8 struct
 *
 * @return A cached OMRUTF8 scope if it is found
 */
const OMRUTF8*
SH_ScopeManagerImpl::findScopeForUTF(OMR_VMThread* currentThread, const OMRUTF8* localScope)
{
 ...
 */
IDATA
SH_ScopeManagerImpl::validate(OMR_VMThread* currentThread, const OMRUTF8* partition, const OMRUTF8* modContext, const ShcItem* match)
{
	const OMRUTF8 *cachedPartition, *cachedModContext;
	...
	if (cachedPartition) {
		if (!(scopedMatch->partitionOffset && (cachedPartition==(const OMRUTF8*)RCWPARTITION(scopedMatch)))) {
	...
	if (cachedModContext) {
		if (!(scopedMatch->modContextOffset && (cachedModContext==(const OMRUTF8*)RCWMODCONTEXT(scopedMatch)))) {
 ...
 */
IDATA
SH_ScopeManagerImpl::createHintsForCachelet(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, CacheletHints* hints)
{
 ...
 */
IDATA
SH_ScopeManagerImpl::primeHashtables(OMR_VMThread* vmthread, SH_CompositeCache* cachelet, U_8* hintsData, UDATA dataLength)
{
